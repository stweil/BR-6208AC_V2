...
{
	unsigned long	flags;
	//printk(KERN_ERR "[DBG HUB]kick khubd\n");
...
static void hub_irq(struct urb *urb)
{
	...
	unsigned i;
	unsigned long bits;
	//printk(KERN_ERR "[DBG HUB]hub irq, status %d\n", status);
	switch (status) {
...
static void hub_events(void)
{
	 ...
	 */
	while (1) {
		//printk(KERN_ERR "[DBG HUB]Start hub event thread\n");
		...
		dev_dbg(hub_dev, "[DBG HUB]state %d ports %d chg %04x evt %04x\n",
		...
		//printk(KERN_ERR "[DBG HUB] deal with port status changes\n");
