...
#define spi_si3220_write8(x,y,z,w)	do{}while(0)
#define spi_si3220_write16(x,y,z,w)	do{}while(0)
#else
...
int __init pcm_init(void)
{
	...
	pcm_outw(RALINK_ANA_CTRL_BASE, 0x00008000);
	pcm_outw(RALINK_ANA_CTRL_BASE+0x0014, 0x01401d61);
	pcm_outw(RALINK_ANA_CTRL_BASE+0x0018, 0x38233d0e);
	pcm_outw(RALINK_ANA_CTRL_BASE+0x001c, 0x80120004);
	pcm_outw(RALINK_ANA_CTRL_BASE+0x0020, 0x1c7dbf48);
	
	/* toggle RG_XPTL_CHG */
	pcm_outw(RALINK_ANA_CTRL_BASE, 0x00008800);
	pcm_outw(RALINK_ANA_CTRL_BASE, 0x00008c00);
	
	data = pcm_inw(RALINK_ANA_CTRL_BASE+0x0014);
	data &= ~(0x0000ffc0);
	...
	data |= REGBIT(0x1, 6);
#endif
	pcm_outw(RALINK_ANA_CTRL_BASE+0x0014, data);
	
	data = pcm_inw(RALINK_ANA_CTRL_BASE+0x0018);
	data &= ~(0xf0773f00);
	data |= REGBIT(0x3, 28);
	data |= REGBIT(0x2, 20);
#if defined(REF_CLK_40MHZ) || defined(REF_CLK_20MHZ)
	...
	data |= REGBIT(0x7, 8);
#endif
	pcm_outw(RALINK_ANA_CTRL_BASE+0x0018, data);

#if defined(REF_CLK_40MHZ) || defined(REF_CLK_20MHZ)
	pcm_outw(RALINK_ANA_CTRL_BASE+0x0020, 0x1c7dbf48);
#else
	pcm_outw(RALINK_ANA_CTRL_BASE+0x0020, 0x1697cc39);
#endif

	//*Common setting - Set PLLGP_CTRL_4 *//
	/* 1. Bit 31 */
	data = pcm_inw(RALINK_ANA_CTRL_BASE+0x001c);
	data &= ~(REGBIT(0x1, 31));
	pcm_outw(RALINK_ANA_CTRL_BASE+0x001c, data);
...
PCM_RESET:	
	...
	if ((slic_type==32261) || (slic_type==32176))
	{		
		if(ProSLIC_HWInit()==0)
		{
			for ( i = 0 ; i < ppcm_config->pcm_ch_num ; i ++ )
...
void pcm_dma_tx_isr(u32 dma_ch)
{
	...
	if ((ppcm_config->nch_active <= 0) && (ppcm_config->tx_isr_cnt >=0 ))
	{
...
void pcm_tx_task(unsigned long pData)
{
	...
	int txch,rxch,ch; 
	int i;
	unsigned short* pTx16Data = NULL;
	unsigned short* pRx16Data = NULL;
	unsigned char* pTx8Data = NULL;
	unsigned char* pRx8Data = NULL;
	unsigned char* p8Data = NULL;
	unsigned long flags;
		...
		for (i = 0 ; i < MAX_PCM_PROC_UNIT ; i ++ )
		{
			...
			if(codec_obj[rxch].type==0)
			{
				...
				for( j = 0 ; j < PCM_8KHZ_SAMPLES ; j++ )		
					pTx16Data[j] = (unsigned short)txpat;
...
#endif					
				memset(pRx8Data, 0, PCM_8KHZ_SAMPLES*PCM_SAMPLE_SIZE);
#else	
				memcpy((void*)pTx8Data, (void*)pRx8Data, PCM_8KHZ_SAMPLES*PCM_SAMPLE_SIZE);
#endif			
			}
...
void pcm_unmask_isr(u32 dma_ch)
{
	...
	if (GDMA_PCM_RX(chid, page)==dma_ch)
	{
		if (page == 0)
			p8Data = (char*)(ppcm_config->RxPage0Buf16Ptr[chid]);
		else
			p8Data = (char*)(ppcm_config->RxPage1Buf16Ptr[chid]);

		GdmaPcmRx((u32)PCM_CH_FIFO(chid), (u32)p8Data, chid, page, PCM_PAGE_SIZE, pcm_dma_rx_isr, pcm_unmask_isr);
		GdmaUnMaskChannel(GDMA_PCM_RX(chid,1-page));
	}
	else
	{	
		if (page == 0)
			p8Data = (char*)(ppcm_config->TxPage0Buf16Ptr[chid]);
		else
			p8Data = (char*)(ppcm_config->TxPage1Buf16Ptr[chid]);
		
		GdmaPcmTx((u32)p8Data, (u32)PCM_CH_FIFO(chid), chid, page, PCM_PAGE_SIZE, pcm_dma_tx_isr, pcm_unmask_isr);
		GdmaUnMaskChannel(GDMA_PCM_TX(chid,1-page));
...
#endif
{
	...
	switch(cmd)
	{
		...
		case PCM_START:
			...
			for( i = 0; i < ptrpcm_config->pcm_ch_num; i++)
				ptrpcm_config->txfifo_rd_idx[i] = 1;

			ptrpcm_config->tx_isr_cnt = -1;
			
			for( i = 0; i < ptrpcm_config->pcm_ch_num; i++)
			{
				pcm_dma_tx_isr(GDMA_PCM_TX(i,0));
				pcm_dma_tx_isr(GDMA_PCM_TX(i,1));
			}
