...
)
{
  ...
  for(i=0; i<MA_NP; i++)
    Copy( &freq_prev_reset[0], &pg729enc->pqua_lsp->freq_prev[i][0], M );
}
...
)
{
  ...
  Relspwed( flsp_in, wegt, lspq_out, lspcb1, lspcb2, fg,
    pg729enc->pqua_lsp->freq_prev, fg_sum, fg_sum_inv, code);
}
...
)
{
  ...
  for(mode = 0; mode<MODE; mode++) {
    Lsp_prev_extract(lsp, rbuf, fg[mode], pg729enc->pqua_lsp->freq_prev, fg_sum_inv[mode]);
  ...
  Lsp_get_quant(lspcb1, lspcb2, cand[mode_index],
      ...
      fg[mode_index], pg729enc->pqua_lsp->freq_prev, lspq, fg_sum[mode_index]) ;
...
)
{
  ...
  for ( i = 0; i < M; i++ ) {
	...
	{
		Word16 psft = abs_s(sft);
		for ( i = 0; i < M; i++ ) {
			wegt[i] = shr(wegt[i], psft);                  /* wegt in Q(11+sft) */
		}
	}
  return;
}
...
void Get_freq_prev(Word16 x[MA_NP][M])
{
  ...
  for (i=0; i<MA_NP; i++)
    Copy(&pg729enc->pqua_lsp->freq_prev[i][0], &x[i][0], M);
}
...
void Update_freq_prev(Word16 x[MA_NP][M])
{
  ...
  for (i=0; i<MA_NP; i++)
    Copy(&x[i][0], &pg729enc->pqua_lsp->freq_prev[i][0], M);
}
