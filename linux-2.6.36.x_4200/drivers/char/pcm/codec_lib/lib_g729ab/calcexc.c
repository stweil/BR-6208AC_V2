...
*/
    ...
    temp1 = extract_h(L_shl(L_acc, sh));  /* fact << (sh+1) */
		...
		temp1 = extract_h(L_shr(L_acc, abs_s(sh)));
    sh = sub(sh, 14);
    if(sh>=0)
    for(i=0; i<L_SUBFR; i++) {
	...
	else
	{
		...
		for(i=0; i<L_SUBFR; i++) {
      		temp2 = mult_r(excg[i], temp1);
      		temp2 = shl(temp2, psh);   /* shl if sh < 0 */
      		excg[i] = temp2;
    	}
    }	
    ...
    if(L_acc < 0) {
      ...
      for(i=0; i<4; i++) {
        ...
        temp1 = shr(excg[(int)pos[i]], sh);
        	...
        	temp1 = shl(excg[(int)pos[i]], abs_s(sh));
        if(sign[i] == 0) {
