...
)
{
 ...
 Word32 t_0, t1, t2;            /* temporary variable                          */

	lpc_type*			plpc = pg729enc->plpc;
	
/* K = A[1] = -R[1] / R[0] */

  t1  = L_Comp(Rh[1], Rl[1]);           /* R[1] in Q31      */
  t2  = L_abs(t1);                      /* abs R[1]         */
  t_0  = Div_32(t2, Rh[0], Rl[0]);       /* R[1]/R[0] in Q31 */
  if(t1 > 0) t_0= L_negate(t_0);          /* -R[1]/R[0]       */
  L_Extract(t_0, &Kh, &Kl);              /* K in DPF         */
  rc[0] = Kh;
  t_0 = L_shr(t_0,4);                     /* A[1] in Q27      */
  L_Extract(t_0, &Ah[1], &Al[1]);        /* A[1] in DPF      */

/*  Alpha = R[0] * (1-K**2) */

  t_0 = Mpy_32(Kh ,Kl, Kh, Kl);          /* K*K      in Q31 */
  t_0 = L_abs(t_0);                       /* Some case <0 !! */
  t_0 = L_sub( (Word32)0x7fffffffL, t_0 ); /* 1 - K*K  in Q31 */
  L_Extract(t_0, &hi, &lo);              /* DPF format      */
  t_0 = Mpy_32(Rh[0] ,Rl[0], hi, lo);    /* Alpha in Q31    */

/* Normalize Alpha */

  alp_exp = norm_l(t_0);
  if(alp_exp>0)
  	t_0 = L_shl(t_0, alp_exp);
  else
  	t_0 = L_shr(t_0, abs_s(alp_exp));
  L_Extract(t_0, &alp_h, &alp_l);         /* DPF format    */
 ...
 *--------------------------------------*/
  ...
  for(i= 2; i<=M; i++)
  {
    ...
    /* t_0 = SUM ( R[j]*A[i-j] ,j=1,i-1 ) +  R[i] */

    t_0 = 0;
    for(j=1; j<i; j++)
      t_0 = L_add(t_0, Mpy_32(Rh[j], Rl[j], Ah[i-j], Al[i-j]));

    t_0 = L_shl(t_0,4);                  /* result in Q27 -> convert to Q31 */
                                       /* No overflow possible            */
    t1 = L_Comp(Rh[i],Rl[i]);
    t_0 = L_add(t_0, t1);                /* add R[i] in Q31                 */

    /* K = -t_0 / Alpha */

    t1 = L_abs(t_0);
    t2 = Div_32(t1, alp_h, alp_l);     /* abs(t_0)/Alpha                   */
    if(t_0 > 0) t2= L_negate(t2);       /* K =-t_0/Alpha                    */
    if(alp_exp>0)
    t2 = L_shl(t2, alp_exp);           /* denormalize; compare to Alpha   */
    else
    	t2 = L_shr(t2, abs_s(alp_exp));	
    ...
    if (sub(abs_s(Kh), 32750) > 0)
    {
      ...
      rc[0] = plpc->old_rc[0];        /* only two rc coefficients are needed */
      rc[1] = plpc->old_rc[1];
      return;
    }
    ...
    for(j=1; j<i; j++)
    {
      t_0 = Mpy_32(Kh, Kl, Ah[i-j], Al[i-j]);
      t_0 = L_add(t_0, L_Comp(Ah[j], Al[j]));
      L_Extract(t_0, &Anh[j], &Anl[j]);
    }
    ...
    t_0 = L_sub( (Word32)0x7fffffffL, t_0 ); /* 1 - K*K  in Q31 */
    L_Extract(t_0, &hi, &lo);              /* DPF format      */
    t_0 = Mpy_32(alp_h , alp_l, hi, lo);   /* Alpha in Q31    */
  ...
  *Err = shr(alp_h, alp_exp);
		...
		*Err = shl(alp_h, abs_s(alp_exp));
  /* Truncate A[i] in Q27 to Q12 with rounding */

  A[0] = 4096;
  for(i=1; i<=M; i++)
  {
    t_0   = L_Comp(Ah[i], Al[i]);
    plpc->old_A[i] = A[i] = round(L_shl(t_0, 1));
  }
  plpc->old_rc[0] = rc[0];
  plpc->old_rc[1] = rc[1];
 ...
 *-------------------------------------------------------------*/
 ...
 {
   Overflow = 0;
   t_0 = L_mult(a[i+1], 16384);          /* x = (a[i+1] + a[M-i]) >> 1        */
   t_0 = L_mac(t_0, a[M-i], 16384);       /*    -> From Q12 to Q11             */
   x  = extract_h(t_0);
   if ( Overflow ) {
     ovf_coef = 1;      }
   ...
   Overflow = 0;
   t_0 = L_mult(a[i+1], 16384);          /* x = (a[i+1] - a[M-i]) >> 1        */
   t_0 = L_msu(t_0, a[M-i], 16384);       /*    -> From Q12 to Q11             */
   x  = extract_h(t_0);
   if ( Overflow ) {
     ovf_coef = 1;      }
 ...
 if ( ovf_coef ) {
   ...
   for (i = 0; i< NC; i++)
   {
     t_0 = L_mult(a[i+1], 8192);          /* x = (a[i+1] + a[M-i]) >> 1        */
     t_0 = L_mac(t_0, a[M-i], 8192);       /*    -> From Q11 to Q10             */
     x  = extract_h(t_0);
     f1[i+1] = sub(x, f1[i]);            /* f1[i+1] = a[i+1] + a[M-i] - f1[i] */

     t_0 = L_mult(a[i+1], 8192);          /* x = (a[i+1] - a[M-i]) >> 1        */
     t_0 = L_msu(t_0, a[M-i], 8192);       /*    -> From Q11 to Q10             */
     x  = extract_h(t_0);
     f2[i+1] = add(x, f2[i]);            /* f2[i+1] = a[i+1] - a[M-i] + f2[i] */
   }
 }
   ...
   if ( L_temp <= (Word32)0)
   {
     ...
     *-------------------------------------------------------------*/
     ...
     else
     {
       ...
       t_0  = L_mult(x, y);
       if(exp<=20)
       	t_0  = L_shr(t_0, sub(20, exp) );
       else
       	t_0  = L_shl(t_0, sub(exp, 20) );
       y   = extract_l(t_0);            /* y= (xhigh-xlow)/(yhigh-ylow) in Q11 */

       if(sign < 0) y = negate(y);

       t_0   = L_mult(ylow, y);                  /* result in Q26 */
       t_0   = L_shr(t_0, 11);                    /* result in Q15 */
       xint = sub(xlow, extract_l(t_0));         /* xint = xlow - ylow*y */
     }
...
static Word16 Chebps_11(Word16 x, Word16 f[], Word16 n)
{
 ...
 /* Note: All computation are done in Q24. */
  ...
  t_0 = L_mult(x, 512);                  /* 2*x in Q24          */
  t_0 = L_mac(t_0, f[1], 4096);           /* + f[1] in Q24       */
  L_Extract(t_0, &b1_h, &b1_l);          /* b1 = 2*x + f[1]     */

  for (i = 2; i<n; i++)
  {
    t_0 = Mpy_32_16(b1_h, b1_l, x);      /* t_0 = 2.0*x*b1              */
    t_0 = L_shl(t_0, 1);
    t_0 = L_mac(t_0,b2_h,(Word16)-32768L);/* t_0 = 2.0*x*b1 - b2         */
    t_0 = L_msu(t_0, b2_l, 1);
    t_0 = L_mac(t_0, f[i], 4096);         /* t_0 = 2.0*x*b1 - b2 + f[i]; */

    L_Extract(t_0, &b0_h, &b0_l);        /* b0 = 2.0*x*b1 - b2 + f[i]; */
  ...
  t_0 = Mpy_32_16(b1_h, b1_l, x);        /* t_0 = x*b1;              */
  t_0 = L_mac(t_0, b2_h,(Word16)-32768L); /* t_0 = x*b1 - b2          */
  t_0 = L_msu(t_0, b2_l, 1);
  t_0 = L_mac(t_0, f[i], 2048);           /* t_0 = x*b1 - b2 + f[i]/2 */

  t_0 = L_shl(t_0, 6);                    /* Q24 to Q30 with saturation */
  cheb = extract_h(t_0);                 /* Result in Q14              */
...
static Word16 Chebps_10(Word16 x, Word16 f[], Word16 n)
{
 ...
 /* Note: All computation are done in Q23. */
  ...
  t_0 = L_mult(x, 256);                  /* 2*x in Q23          */
  t_0 = L_mac(t_0, f[1], 4096);           /* + f[1] in Q23       */
  L_Extract(t_0, &b1_h, &b1_l);          /* b1 = 2*x + f[1]     */
  ...
  t_0 = L_shl(t_0, 7);                    /* Q23 to Q30 with saturation */
  cheb = extract_h(t_0);                 /* Result in Q14              */
