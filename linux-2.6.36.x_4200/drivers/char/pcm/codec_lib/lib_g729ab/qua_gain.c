...
)
{
 ...
 static Word16 past_qua_en[4] = { -14336, -14336, -14336, -14336 };
   ...
   *---------------------------------------------------*/
   ...
   Gain_predict( pg729enc->pqua_gain->past_qua_en, code, L_subfr, &gcode0, &exp_gcode0 );
   ...
   *-----------------------------------------------------------------*/
   ...
   denom = extract_h( L_shl(L_tmp, sft) );
   	...
   	denom = extract_h( L_shr(L_tmp, abs_s(sft)) );
   ...
   *-----------------------------------------------------------------*/
   ...
   nume = extract_h( L_shl(L_tmp, sft) );
   	...
   	nume = extract_h( L_shr(L_tmp, abs_s(sft)) );
   ...
   L_acc = L_shr( L_mult( nume,inv_denom ), sft );
    ...
    L_acc = L_shl( L_mult( nume,inv_denom ), abs_s(sft) );
   ...
   *-----------------------------------------------------------------*/
   ...
   if( sub(exp1, exp2)>0 ){
   		Word16 shift = add(sub(exp1,exp2),1);
   		if(shift>=0)
      		L_tmp = L_sub( L_shr( L_tmp1, shift ), L_shr( L_tmp2,1 ) );
      	else
      		L_tmp = L_sub( L_shl( L_tmp1, abs_s(shift) ), L_shr( L_tmp2,1 ) );	
      exp = sub(exp2,1);
   }
   else{
   		Word16 shift = add(sub(exp2,exp1),1);
   		if(shift>=0)
      		L_tmp = L_sub( L_shr( L_tmp1,1 ), L_shr( L_tmp2, shift ) );
      	else
      		L_tmp = L_sub( L_shr( L_tmp1,1 ), L_shl( L_tmp2, abs_s(shift) ) );	
      exp = sub(exp1,1);
   }
   sft = norm_l( L_tmp );
   if(sft>=0)
...
if(tameflag == 1){
   ...
   *-----------------------------------------------------------------*/
   ...
   {
   	Word16 shitf = add( negate(exp_gcode0),(-12-1+1+16) );
    if(shitf>=0)
    	L_acc = L_shl(L_acc, shitf);
    else
    	L_acc = L_shr(L_acc, abs_s(shitf));	
   }
   ...
   *----------------------------------------------*/
   Gain_update( pg729enc->pqua_gain->past_qua_en, L_gbk12 );
