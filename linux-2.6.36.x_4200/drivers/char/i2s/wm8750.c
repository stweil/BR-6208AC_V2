 ...
 *                     \/            \/     \/    \/            \/
 * $Id: //WIFI_SOC/MP/SDK_4_2_0_0/RT288x_SDK/source/linux-2.6.21.x/drivers/char/i2s/wm8750.c#3 $
 *
...
void audiohw_postinit(int bSlave, int AIn, int AOut, int wordLen24b)
{
	...
	{ 
		...
		if (wordLen24b)
		{
			...
			wmcodec_write(AINTFCE, AINTFCE_WL_24 | AINTFCE_FORMAT_I2S);
		}
		else
		{
	...
	else
	{
		...
		{
			printk("24 bit word length\n");
			wmcodec_write(AINTFCE, AINTFCE_MS | AINTFCE_WL_24 | AINTFCE_FORMAT_I2S);
		}
		else
		{
			printk("16 bit word length\n");
