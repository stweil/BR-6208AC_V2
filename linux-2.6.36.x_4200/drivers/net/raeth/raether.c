...
#include "../../../../kernel_module/net/hw_nat/ra_nat.h" //EDX
#endif
	...
	*/
	...
	else if((recv == 0) && (transmit == 0))
	{
		...
		if(reg_int_val & BIT(25))
		{
			...
			{
				transmit = (sysRegRead(RALINK_ETH_SW_BASE+0x0208)>>2 & 0x3);
				if(transmit == 1)
				{
...
#ifdef _EW7478AC_ //EDX Patrick, fixed RTL8211E giga phy issue
/**reset phy
 * 1. set gpio1 to output
 * 2. set gpio1 to low
 * 3. delay 30 ms
 * 4. set gpio1 to high
 * 5. finish
...
{
	printk("======>  reset_phy_by_gpio39()\n");
	#define RALINK_GPIO_DIR	0xB0000600+0x4C
	#define RALINK_GPIO_DATA	0xB0000600+0x48
	#define RALINK_RESETPHY_GPIO  15//(39-24)//0

	*(unsigned long *)(RALINK_GPIO_DIR) |= (1<<RALINK_RESETPHY_GPIO); 
	*(unsigned long *)(RALINK_GPIO_DATA) &= ~(1<<RALINK_RESETPHY_GPIO); //set to low
	udelay(10000);
	*(unsigned long *)(RALINK_GPIO_DATA) |= (1<<RALINK_RESETPHY_GPIO); //set to high
...
void rt_gsw_init(void)
{
  ...
  *(unsigned long *)(RALINK_ETH_SW_BASE+0x3500) = 0x5e33b;//(P5, Force mode, Link Up, 1000Mbps, Full-Duplex, FC ON)
	...
	*(unsigned long *)(SYSCFG1) |= (0x1 << 12);
	...
	#ifdef _EW7478AC_ //EDX Patrick, fixed RTL8211E giga phy issue
	printk("======>  Diable internal phy\n");
	*(unsigned long *)(RALINK_ETH_SW_BASE+0x7014) |= (0x1f<<24);//Diable internal phy
	//printk("======>  Diable port0-port4 internal phy \n");
	//*(unsigned long *)(RALINK_ETH_SW_BASE+0x7014) |= (0x1f<<24); //disable port0-port4 internal phy
	//printk("<======  Diable port0-port4 internal phy \n");
	#endif
	...
	*(unsigned long *)(SYSCFG1) &= ~(0x3 << 12); //GE1_MODE=RGMii Mode
	...
	#ifdef _EW7478AC_ //EDX Patrick, fixed RTL8211E giga phy issue
	printk("======>  reset_reltek_phy\n");
	reset_reltek_phy();//reset_phy_by_gpio39();
	#endif
