...
RTMP_REG_PAIR mt76x2_mac_g_band_cr_table[] = {
	...
	{PAMODE_PWR_ADJ0, 0xE600F300},
	{PAMODE_PWR_ADJ1, 0xE600F300},
...
void do_calibrtion_info_for_robust(RTMP_ADAPTER *ad, UINT32 cal_id, UINT8 channel)
{
	...
	switch(cal_id) {
		...
		case RXIQC_FI_CALIBRATION_7662:
			...
			break;
		case RXIQC_FD_CALIBRATION_7662:
			DBGPRINT(RT_DEBUG_OFF, ("\nnot supoort RXIQC_FD_CALIBRATION_7662 yet%d\n"));
			break; 
		default:
			DBGPRINT(RT_DEBUG_OFF, ("\nunknbow cal id = %d\n", cal_id));
			break;
...
void mt76x2_adjust_per_rate_pwr_delta(RTMP_ADAPTER *ad, u8 channel)
{
	u32 value;
...
static void mt76x2_switch_channel(RTMP_ADAPTER *ad,
								  u8 channel, BOOLEAN scan)
{
	RTMP_CHIP_CAP *cap = &ad->chipCap;
	...
	/* tune init gain to 0x58 for channel model E */	
	RTMP_BBP_IO_READ32(ad, AGC1_R8, &value);
	value &= 0xffff80ff;
	value |= ((0x7f & 0x58) << 8);
	RTMP_BBP_IO_WRITE32(ad, AGC1_R8, value);

	RTMP_BBP_IO_READ32(ad, AGC1_R9, &value);
	value &= 0xffff80ff;
	value |= ((0x7f & 0x58) << 8);
	RTMP_BBP_IO_WRITE32(ad, AGC1_R9, value);
...
static void mt76x2_init_mac_cr(RTMP_ADAPTER *ad)
{
	...
	*/
	 ...
	 */
	if (IS_MT76x2E(ad) || IS_MT7612U(ad)) {	
		RTMP_IO_WRITE32(ad, 0x504, 0x7f00000);
		RTMP_IO_WRITE32(ad, 0x50c, 0x1a800000);
		RtmpOsMsDelay(5);
		RTMP_IO_WRITE32(ad, 0x504, 0x0);
		RTMP_IO_WRITE32(ad, 0x50c, 0x0);
	}
...
static BOOLEAN mt76x2_is_cal_free_ic(RTMP_ADAPTER *ad)
{
	...
	eFuseReadRegisters(ad, GRP0_TX0_A_BAND_TARGET_PWR, 2, &TxPower);
	eFuseReadRegisters(ad, GRP0_TX0_A_BAND_TSSI_SLOPE, 2, &TssiSlope);
...
static VOID mt76x2_cal_free_data_get(RTMP_ADAPTER *ad)
{
	...
	ad->EEPROMImage[XTAL_TRIM1] = value & XTAL_TRIM1_MASK;

	/* 0x55 0x56 0x57 0x5C 0x5D */
	eFuseReadRegisters(ad, A_BAND_EXT_PA_SETTING, 2, &value);
	ad->EEPROMImage[A_BAND_EXT_PA_SETTING + 1] = (value & TEMP_SENSOR_CAL_MASK) >> 8;
