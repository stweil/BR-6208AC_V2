...
void clean(char *path, char *extension, char *type)
{
	...
	if( (dir=opendir(pathname)) != NULL )
	{
		while( (ptr = readdir(dir))!=NULL )
		{
			...
			if( strlen(ptr->d_name)>strlen(tempstr) && !strcmp( ptr->d_name+strlen(ptr->d_name)-strlen(tempstr), tempstr ) && (file=fopen(filename, "r")) != NULL && (file_temp=fopen("/etc/clean_tmp", "w")) != NULL )
			{
