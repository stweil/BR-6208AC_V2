    ...
    IN PRTMP_ADAPTER pAd, IN ULONG i)
{
	...
	{
		UINT tx_cnt_threshold = EdccaAssocStaThroughputJudge(pAd, pEntry, pCurrTxRate, TxSuccess);
		if(TxSuccess >= tx_cnt_threshold)
			pAd->ed_has_traffic_sta_cnt++;
		if(pAd->ed_debug)
			DBGPRINT(RT_DEBUG_ERROR 
			,("%s(): TxTotalCnt: %ld  TxSuccess: %d	 tx_cnt_threshold: %d pCurrTxRate->CurrMCS: %d pCurrTxRate->BW :%d  pCurrTxRate->Mode: %d\n"
			, __FUNCTION__, TxTotalCnt,TxSuccess,tx_cnt_threshold,pCurrTxRate->CurrMCS,pCurrTxRate->BW,pCurrTxRate->Mode));
	}
#endif /* ED_MONITOR */
