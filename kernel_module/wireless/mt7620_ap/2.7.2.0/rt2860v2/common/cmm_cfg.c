...
{	
	...
	)
	{
		if( pAd->ed_current_state == ED_OFF_AND_LEARNING )
		{
			...
			RTMPSetTimer(&pAd->ed_timer, 1800000);
		}
		else if( pAd->ed_current_state == ED_TESTING )
		{
			//reset timer to 10 mins
			BOOLEAN cancelled;
			RTMPCancelTimer(&pAd->ed_timer, &cancelled);
			RTMPSetTimer(&pAd->ed_timer, 1800000);
			DBGPRINT(RT_DEBUG_TRACE,("\n\n@@@@@@ %s : reset ed_timer to 10 mins !!\n\n",__FUNCTION__));			
		}
	}
...
#if 0//def RT6352
	if(IS_RT6352(pAd))
		Set_DyncVgaEnable_Proc(pAd,"1"); /* FOR 7620 only !!*/
#endif
...
#if 0//def RT6352
	if(IS_RT6352(pAd))
		Set_DyncVgaEnable_Proc(pAd,"0"); /* FOR 7620 only !!*/
#endif
	RTMP_IRQ_LOCK(&pAd->irq_lock, irqflag);
	DBGPRINT(RT_DEBUG_OFF, ("@@@ %s : ===>\n", __FUNCTION__));
	NdisZeroMemory(&pAd->ed_stat[0], sizeof(pAd->ed_stat));
