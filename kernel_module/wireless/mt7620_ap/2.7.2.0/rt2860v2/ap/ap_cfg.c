...
//EDX yihong, for new Adaptivity patch_20151029
//INT set_ed_rssi_gap_threhold_proc(RTMP_ADAPTER *pAd, PSTRING arg);
//INT set_ed_tput_threhold_proc(RTMP_ADAPTER *pAd, PSTRING arg);
...
//EDX yihong, for new Adaptivity patch_20151029
//	{"ed_rssi_gap", set_ed_rssi_gap_threhold_proc},
//	{"ed_tput", set_ed_tput_threhold_proc},
...
/******/
	...
	for (i=0; i<MaxWcidNum; i++)
	{
			...
			&& (pEntry->Sst == SST_ASSOC))
		{
			...
			sprintf(msg+strlen(msg),"\t\t\t\t\t\t\t%-10s", GetPhyMode(pEntry->MaxHTPhyMode.field.MODE));
			sprintf(msg+strlen(msg),"%-6s", GetBW(pEntry->MaxHTPhyMode.field.BW));
#ifdef DOT11_VHT_AC
        ...
        IN      RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	NdisZeroMemory(Profile.WscSsid, 32);
	NdisMoveMemory(Profile.WscSsid, pAd->ApCfg.ApCliTab[CurApIdx].Ssid, pAd->ApCfg.ApCliTab[CurApIdx].SsidLen);
	Profile.WscSsidLen = pAd->ApCfg.ApCliTab[CurApIdx].SsidLen;


	Profile.WscAuthMode = WscGetAuthType(pAd->ApCfg.ApCliTab[CurApIdx].AuthMode);

	Profile.WscEncrypType = WscGetEncryType(pAd->ApCfg.ApCliTab[CurApIdx].WepStatus);
	...
	if (Profile.WscEncrypType != WSC_ENCRTYPE_NONE)
	{
		if (Profile.WscEncrypType & (WSC_ENCRTYPE_TKIP | WSC_ENCRTYPE_AES))
		{
			...
			if (pAd->ApCfg.ApCliTab[CurApIdx].WscControl.WpaPskLen >= 8 && pAd->ApCfg.ApCliTab[CurApIdx].WscControl.WpaPskLen <= 64)
			{
				NdisZeroMemory(Profile.WscWPAKey, 64);
				NdisMoveMemory(Profile.WscWPAKey, pAd->ApCfg.ApCliTab[CurApIdx].WscControl.WpaPsk, pAd->ApCfg.ApCliTab[CurApIdx].WscControl.WpaPskLen);
				Profile.WscWPAKeyLen=pAd->ApCfg.ApCliTab[CurApIdx].WscControl.WpaPskLen;
				DBGPRINT(RT_DEBUG_TRACE, ("WpaPskLen = %d\n", pAd->ApCfg.ApCliTab[CurApIdx].WscControl.WpaPskLen));
			}
			else
			{
				DBGPRINT(RT_DEBUG_TRACE, ("WPAPSK: Invalid Key Length (%d)\n", pAd->ApCfg.ApCliTab[CurApIdx].WscControl.WpaPskLen));
				pAd->ApCfg.ApCliTab[CurApIdx].WscControl.WpaPskLen = 0;
			}
		}
	}
	...
	for(i=0; i < MaxWcidNum; i++)
        {
			...
			if(!strcmp(temp_MAC, Profile.AP_MAC))
			{
				printk("pEntry->HTPhyMode.field.BW=%02X\n", pEntry->HTPhyMode.field.BW);
                        	//printk("\nGetBW(pEntry->HTPhyMode.field.BW)=%s", GetBW(pEntry->HTPhyMode.field.BW));
	...
	IN	PSTRING		arg)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("Set_DyncVgaEnable_Proc::(enable = %d)\n", pAd->CommonCfg.MO_Cfg.bDyncVGAEnable));
