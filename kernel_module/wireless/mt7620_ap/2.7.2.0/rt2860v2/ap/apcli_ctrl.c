...
extern INT EDXEvent(IN USHORT Event, char *pifname);
#endif
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	{
#ifdef EDX_NOTIFY //EDX Randall for repeater
		EDXEvent(IW_STA_LINKUP_EVENT_FLAG, pAd->net_dev->name);
#endif
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if(Status == MLME_SUCCESS)
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("(%s) Auth Rsp Success.\n", __FUNCTION__)); //EDX patrick
	...
	else
	{
		if (pApCliEntry->AuthMode == Ndis802_11AuthModeAutoSwitch)
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("(%s) Auth Rsp Failure.\n", __FUNCTION__)); //EDX patrick
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if(Status == MLME_SUCCESS)
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("(%s) Auth2 Rsp Success.\n", __FUNCTION__)); //EDX patrick
		*pCurrState = APCLI_CTRL_ASSOC;
	...
	else
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("(%s) Apcli Auth Rsp Failure.\n", __FUNCTION__)); //EDX patrick
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if(Status == MLME_SUCCESS)
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("(%s) apCliIf = %d, Receive Assoc Rsp Success.\n", __FUNCTION__, ifIndex)); //EDX patrick
	...
	else
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("(%s) apCliIf = %d, Receive Assoc Rsp Failure.\n", __FUNCTION__,  ifIndex)); //EDX patrick
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("(%s) MLME Request disconnect.\n", __FUNCTION__)); //EDX patrick
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("(%s) Peer DeAssoc Req.\n", __FUNCTION__)); //EDX patrick
