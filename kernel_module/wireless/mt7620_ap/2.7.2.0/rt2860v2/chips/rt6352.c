...
RTMP_REG_PAIR	RT6352_MACRegTable[] =	{
	...
	{0x1030,  			0x66655443}, //Neil,2015-02-03
	{0x1034,  			0x77766554}, //Neil,2015-02-03
	{0x1038,  			0x77766554}, //Neil,2015-02-03
	{0x103C,  			0x77777665}, //Neil,2015-02-03	
};
	...
	IN PRTMP_ADAPTER pAd)
{
	if ((pAd->CommonCfg.MO_Cfg.bDyncVGAEnable) && (pAd->bCalibrationDone))
	{
		if ((pAd->Mlme.PeriodicRound % MLME_TASK_EXEC_MULTIPLE) == 0)
		{
			...
			if (((pAd->chipCap.avg_rssi_all <= -79) && (pAd->CommonCfg.BBPCurrentBW == BW_40))
			...
			if (pAd->RalinkCounters.OneSecFalseCCACnt > pAd->CommonCfg.MO_Cfg.nFalseCCATh)
			{
				...
				if (BbpReg < (pAd->CommonCfg.MO_Cfg.Stored_BBP_R66 + VgaGainLowerBound))
				{
					BbpReg += 4;
					AsicBBPWriteWithRxChain(pAd, BBP_R66, BbpReg, RX_CHAIN_ALL);
				}
				else if (BbpReg == (pAd->CommonCfg.MO_Cfg.Stored_BBP_R66 + VgaGainLowerBound))
				{
