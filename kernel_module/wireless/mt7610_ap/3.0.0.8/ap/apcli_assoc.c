	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	{		
		...
		VarIesOffset += 1;
		NdisMoveMemory(pApCliEntry->ReqVarIEs + VarIesOffset, &pApCliEntry->ApCliMlmeAux.SsidLen, 1);
		VarIesOffset += 1;
		NdisMoveMemory(pApCliEntry->ReqVarIEs + VarIesOffset, pApCliEntry->ApCliMlmeAux.Ssid, pApCliEntry->ApCliMlmeAux.SsidLen);
		VarIesOffset += pAd->MlmeAux.SsidLen;

		/*Second add Supported rates*/
		NdisMoveMemory(pApCliEntry->ReqVarIEs + VarIesOffset, &SupRateIe, 1);
		VarIesOffset += 1;
		NdisMoveMemory(pApCliEntry->ReqVarIEs + VarIesOffset, &pApCliEntry->ApCliMlmeAux.SupRateLen, 1);
		VarIesOffset += 1;
		NdisMoveMemory(pApCliEntry->ReqVarIEs + VarIesOffset, pApCliEntry->ApCliMlmeAux.SupRate, pApCliEntry->ApCliMlmeAux.SupRateLen);
		VarIesOffset += pApCliEntry->ApCliMlmeAux.SupRateLen;
#endif /* APCLI_WPA_SUPPLICANT_SUPPORT */
		...
		{
			ULONG TmpLen;
			UCHAR RalinkIe[11] = {IE_VENDOR_SPECIFIC, 9, 0x00, 0x07, 0x40, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00};
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, 11, RalinkIe, END_OF_ARGS);
			FrameLen += TmpLen;
		}
#endif
		...
		*/
            ...
            )
		{
			...
			/* Search chched PMKID, append it if existed */
				...
				*/
				if ((pApCliEntry->AuthMode == Ndis802_11AuthModeWPA2) &&
					(NdisEqualMemory(pApCliEntry->ApCliMlmeAux.Bssid, pAd->CommonCfg.LastBssid, MAC_ADDR_LEN)))
				{
					FoundPMK = FALSE;
				}
...
#endif
			{
				...
				VarIesOffset += pApCliEntry->RSNIE_Len;
...
#ifdef APCLI_WPA_SUPPLICANT_SUPPORT
	if (pApCliEntry->WpaSupplicantUP != WPA_SUPPLICANT_DISABLE) 
	{
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	{
		/* The frame is for me ? */
		if(MAC_ADDR_EQUAL(Addr2, pApCliEntry->ApCliMlmeAux.Bssid))
		{
...
#ifdef DOT11_VHT_AC
		                        RTMPZeroMemory(&pApCliEntry->ApCliMlmeAux.vht_cap, sizeof(VHT_CAP_IE));
					RTMPZeroMemory(&pApCliEntry->ApCliMlmeAux.vht_op, sizeof(VHT_OP_IE));
					pApCliEntry->ApCliMlmeAux.vht_cap_len = 0;
					pApCliEntry->ApCliMlmeAux.vht_op_len = 0;
		                        if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) && ie_list->vht_cap_len && ie_list->vht_op_len)
		                        {						
						NdisMoveMemory(&pApCliEntry->ApCliMlmeAux.vht_cap, &(ie_list->vht_cap), ie_list->vht_cap_len);
						pApCliEntry->ApCliMlmeAux.vht_cap_len = ie_list->vht_cap_len;
						NdisMoveMemory(&pApCliEntry->ApCliMlmeAux.vht_op, &(ie_list->vht_op), ie_list->vht_op_len);
						pApCliEntry->ApCliMlmeAux.vht_op_len = ie_list->vht_op_len;
		                        }							
#endif /* DOT11_VHT_AC */
				}
