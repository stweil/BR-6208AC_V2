...
#ifdef MAC_REPEATER_SUPPORT
INT Set_BssRepeaterGid_Proc(
	IN RTMP_ADAPTER		*pAd,
	IN PSTRING			arg);

INT Set_EthRepeaterGid_Proc(
	IN RTMP_ADAPTER		*pAd,
	IN PSTRING			arg);
#endif /* MAC_REPEATER_SUPPORT */
...
static struct {
	...
	{"ATEENABLEFLASH",				Set_ATE_ENABLE_FLASH_Proc},
	{"ATECALFLASH",					Set_ATE_CAL_FLASH_Proc},
...
#ifdef IP_ASSEMBLY
	{"force_pkt_ass",				set_force_ip_assemble},
#endif /* IP_ASSEMBLY */
#endif /* WFA_VHT_PF */
...
#ifdef MAC_REPEATER_SUPPORT
	{"BssRepeaterGid",	Set_BssRepeaterGid_Proc},
	{"EthRepeaterGid",	Set_EthRepeaterGid_Proc},
#endif /* MAC_REPEATER_SUPPORT */

#ifdef MT76x0
	{"DoTemperatureSensor",	Set_DoTemperatureSensor_Proc},
...
//	{
	this_char = pIoctlCmdStr->u.data.pointer; //EDX Leo

		if (!*this_char){	//EDX Leo
			//continue;	//EDX Leo
		}
	...
	IN	PSTRING			arg)
{
	...
	for (McsIdx =0 ; McsIdx < vht_mcs_max_idx; McsIdx++)
		...
		for (i = start; i != stop;  i = (i+1) % DIAGNOSE_TIME)
		{
			printk("%-7d", pDiag->diag_info[i].TxSGICnt_VHT[McsIdx]);
		}
		printk("\n");
	}
#endif /* DOT11_VHT_AC */
	...
	for (McsIdx =0 ; McsIdx < vht_mcs_max_idx; McsIdx++)
	{
		...
		for (i = start; i != stop;  i = (i+1) % DIAGNOSE_TIME)
		{
			printk("%-7d", pDiag->diag_info[i].RxSGICnt_VHT[McsIdx]);
		}
		printk("\n");
        ...
        IN      RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	if (copy_to_user(wrq->u.data.pointer, &(pApCliTab->WscControl.WscProfile), wrq->u.data.length))
	{
		DBGPRINT(RT_DEBUG_TRACE, ("%s: copy_to_user() fail\n", __FUNCTION__));
	}
	...
	/*if (pAd->ApCfg.MBSSID[apidx].AuthMode == Ndis802_11AuthModeWPA1PSKWPA2PSK)
		...
		Profile.WscAuthMode = WscGetAuthType(pApCliTab->AuthMode);
	/*if (pAd->ApCfg.MBSSID[apidx].WepStatus == Ndis802_11Encryption4Enabled)
		Profile.WscEncrypType = WSC_ENCRTYPE_TKIP |WSC_ENCRTYPE_AES;
	else*/
		Profile.WscEncrypType = WscGetEncryType(pApCliTab->WepStatus);
...
#ifdef MAC_REPEATER_SUPPORT
/*
	Set MAC REPEATER Group Index for BSS
	(i.e. STAs connects to rax would associate to Root AP via that apcli interface.)
*/
INT Set_BssRepeaterGid_Proc(
	IN RTMP_ADAPTER		*pAd,
	IN PSTRING			arg)
{
	...
	{
		pMbss->ApCliIdx = UNKOWN_APCLI_IDX;
		DBGPRINT(RT_DEBUG_WARN, ("Set_BssRepeaterGid_Proc(%d):: MAC Repeater is disabled\n", pAd->ApCfg.bMACRepeaterEn));
	}
	else
	{
		if (val == UNKOWN_APCLI_IDX)
			pMbss->ApCliIdx = val; /* Disable MAC Repeater of this BSS */
		else if (val <= pAd->ApCfg.ApCliNum)
			pMbss->ApCliIdx = val; /* Enable MAC Repeater of this BSS */
		else
			DBGPRINT(RT_DEBUG_WARN, ("Set_BssRepeaterGid_Proc:: Unkown value. Keep original value.\n"));
	}

	DBGPRINT(RT_DEBUG_TRACE, ("Set_BssRepeaterGid_Proc:: ApCliIdx = %d \n", pMbss->ApCliIdx));

	return TRUE;
}

/*
	Set MAC REPEATER Group Index for Ethernet PC/NB
*/
INT Set_EthRepeaterGid_Proc(
	IN RTMP_ADAPTER		*pAd,
	IN PSTRING			arg)
{
	...
	if (pAd->ApCfg.bMACRepeaterEn == FALSE)
	{
		...
		DBGPRINT(RT_DEBUG_WARN, ("Set_EthRepeaterGid_Proc(%d):: MAC Repeater is disabled\n", pAd->ApCfg.bMACRepeaterEn));
	}
	else
	{
		if (val == UNKOWN_APCLI_IDX)
			pAd->ApCfg.EthApCliIdx = val; /* Disable MAC Repeater of ethernet PC/NB */
		else if (val <= pAd->ApCfg.ApCliNum)
			pAd->ApCfg.EthApCliIdx = val; /* Enable MAC Repeater of ethernet PC/NB */
		else
			DBGPRINT(RT_DEBUG_WARN, ("Set_BssRepeaterGid_Proc:: Unkown value. Keep original value.\n"));
	}

	DBGPRINT(RT_DEBUG_TRACE, ("Set_EthRepeaterGid_Proc:: EthApCliIdx = %d \n", pAd->ApCfg.EthApCliIdx));
	...
	IN	PSTRING		arg)
{
	...
	if (pAd->CommonCfg.MO_Cfg.bDyncVgaEnable == FALSE)
	{
