...
#ifdef _Buffalo_ // Don't send beacons when not connected with Band 2/3 root AP.
		DBGPRINT(RT_DEBUG_OFF, ("[Buffalo] APCLI LINK UP: Channel = %d      pAd->Dot11_H.RDMode %s RD_NORMAL_MODE\n", pAd->CommonCfg.Channel, (pAd->Dot11_H.RDMode==RD_NORMAL_MODE)?"==":"!="));
		if( pAd->CommonCfg.Channel > 48 && pAd->Dot11_H.RDMode == RD_NORMAL_MODE )
		{
			DBGPRINT(RT_DEBUG_OFF, ("[Buffalo] APCLI LINK UP: Enable Beacon\n"));
			BuffaloEnableBeacon(pAd);
		}
#endif
			}
			else
...
#ifdef _Buffalo_ // Don't send beacons when not connected with Band 2/3 root AP.
		DBGPRINT(RT_DEBUG_OFF, ("[Buffalo] APCLI LINK DOWN: Channel = %d\n", pAd->CommonCfg.Channel));
		if( pAd->CommonCfg.Channel > 48 )
		{
			DBGPRINT(RT_DEBUG_OFF, ("[Buffalo] APCLI LINK DOWN: Disable Beacon\n"));
			BuffaloDisableBeacon(pAd);
		}
		}
#endif
	}
