...
#ifdef ED_MONITOR
{
	...
	#ifdef _ADV_CONTROL_ //EDX patrick
	if(advControl & 1){
		printk("\n@@@ 7610 adaptivtyControl ON !!\n");
		if(advControl & 4){
			printk("@@@ 7610 adaptivtyControl Enable ed_monitor!!\n");
			bEdcca = TRUE;
		}
		else{
			printk("@@@ 7610 adaptivtyControl Disable ed_monitor!!\n");
			bEdcca = FALSE;
		}
	}
	#endif
...
#ifdef _Buffalo_
	//DBGPRINT(RT_DEBUG_OFF,("LOG# %02x:%02x:%02x:%02x:%02x:%02x %s\n", pAddr[0], pAddr[1], pAddr[2], pAddr[3], pAddr[4], pAddr[5], pEventText[Event]));
#endif
}
#endif /* SYSTEM_LOG_SUPPORT */
