...
/*			DBGPRINT(RT_DEBUG_OFF, ("%02X:%02X:%02X:%02X:%02X:%02X "
			...
			if( pNetDev->priv_flags == IFF_EBRIDGE )
				NdisMoveMemory(&uuid_t.node[0], &pNetDev->dev_addr[0], sizeof(uuid_t.node));
			//DBGPRINT(RT_DEBUG_OFF, ("%c\n", ((pNetDev->priv_flags == IFF_EBRIDGE)?'Y':'N')));
		}
		//DBGPRINT(RT_DEBUG_OFF, ("<=7610===================\n"));
/*
		UINT32 tempval;
		tempval =
			( *(UINT8 *)(&uuid_t.node[2]) << 0 ) +
			( *(UINT8 *)(&uuid_t.node[3]) << 8 ) +
			( *(UINT8 *)(&uuid_t.node[4]) << 16 ) +
			( *(UINT8 *)(&uuid_t.node[5]) << 24 );

		tempval -= 2;

		uuid_t.node[2] = (tempval >> 0) & 0xFF;
		uuid_t.node[3] = (tempval >> 8) & 0xFF;
		uuid_t.node[4] = (tempval >> 16) & 0xFF;
		uuid_t.node[5] = (tempval >> 24) & 0xFF;
*/
#endif
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	DBGPRINT(RT_DEBUG_OFF, ("<7610>The UUID Hex string is:"));
	for (i=0; i< 16; i++)
	{
		DBGPRINT(RT_DEBUG_OFF, ("%02x", (uuidHexStr[i] & 0xff)));
	}
	DBGPRINT(RT_DEBUG_OFF, ("\n"));
	DBGPRINT(RT_DEBUG_OFF, ("<7610>The UUID ASCII string is:%s!\n", uuidAscStr));
	return 0;
}
