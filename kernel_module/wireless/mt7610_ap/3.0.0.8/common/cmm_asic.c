...
/*	RTMP_IO_WRITE32(pAd, BCN_TIME_CFG, 0x00000000);*/
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		csr.field.bBeaconGen  = (pAd->CommonCfg.Channel <= 48)?1:0; /* AP should generate BEACON*/
		DBGPRINT(RT_DEBUG_OFF, ("[Buffalo] AsicEnableBssSync: Channel = %d   ===>   csr.field.bBeaconGen = %d\n", pAd->CommonCfg.Channel, csr.field.bBeaconGen));
#else
...
#ifdef _Buffalo_ // Don't send beacons when not connected with Band 2/3 root AP.
VOID BuffaloDisableBeacon(
	IN PRTMP_ADAPTER pAd)
{
	BCN_TIME_CFG_STRUC csr;

	DBGPRINT(RT_DEBUG_TRACE, ("--->BuffaloDisableBeacon\n"));
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		//csr.field.bTsfTicking = 1;
		//csr.field.TsfSyncMode = 3; /* sync TSF similar as in ADHOC mode?*/
		csr.field.bBeaconGen  = 0; /* AP should generate BEACON*/
		//csr.field.bTBTTEnable = 1;
	}
#endif /* CONFIG_AP_SUPPORT */
	RTMP_IO_WRITE32(pAd, BCN_TIME_CFG, csr.word);
...
VOID BuffaloEnableBeacon(
	IN PRTMP_ADAPTER pAd)
{
	BCN_TIME_CFG_STRUC csr;

	DBGPRINT(RT_DEBUG_TRACE, ("--->BuffaloEnableBeacon\n"));
