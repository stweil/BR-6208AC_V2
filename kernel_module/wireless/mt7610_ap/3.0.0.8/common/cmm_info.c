...
PSTRING GetEncryptType(CHAR enc)
{
	...
	if(enc == Ndis802_11Encryption4Enabled)
    	return "AESTKIP"; //return "TKIPAES"; //EDX patrick
#ifdef WAPI_SUPPORT
 	...
 	IN	PSTRING			arg)
{
	...
	if (pAd->ApCfg.bMACRepeaterEn)
 		...
 		&& (pAd->ApCfg.ApCliTab[ifIndex].SsidLen != 0))
 	{
 			...
 			if ( IS_ENTRY_APCLI(pEntry)
				...
				&& (MAC_ADDR_EQUAL(pEntry->Addr, pApCliEntry->ApCliMlmeAux.Bssid))
 				...
 				{ 
 					...
 					DBGPRINT(RT_DEBUG_OFF, ("ApClii%d Connected AP : %02X:%02X:%02X:%02X:%02X:%02X   SSID:%s   ProtStatus:%s\n",ifIndex,
 						pEntry->Addr[0], pEntry->Addr[1], pEntry->Addr[2],
 						pEntry->Addr[3], pEntry->Addr[4], pEntry->Addr[5],
		...
		if (!bConnect)
			DBGPRINT(RT_DEBUG_OFF, ("ApClii%d Connected AP : Disconnect\n",ifIndex));
 	}
 	else
 	{
 		DBGPRINT(RT_DEBUG_OFF, ("ApClii%d Connected AP : Disconnect\n",ifIndex));
...
INT set_tr_stop(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	pAd->fpga_tr_stop = (stop <= 4 ? stop : 0);
	DBGPRINT(RT_DEBUG_TRACE, ("%s(): fpga_tr_stop=0x%x\n", __FUNCTION__, pAd->fpga_tr_stop));
...
INT set_rf(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	if (arg)
	{
		...
		if (rv == 3)
		{
			...
			DBGPRINT(RT_DEBUG_OFF, ("%s():%d %03d 0x%02X\n", __FUNCTION__, bank_id, rf_id, rf_val));
		}
		else if (rv == 2)
		{
			rlt_rf_read(pAd, bank_id, rf_id, &rf_val);
