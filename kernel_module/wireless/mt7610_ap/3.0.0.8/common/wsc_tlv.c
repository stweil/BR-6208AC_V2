...
extern UCHAR SecondProfileSsid[MAX_LEN_OF_SSID+1];
extern UCHAR SecondProfileMac[6];
extern USHORT SecondProfileAuthMode;
extern USHORT SecondProfileEncrType;
extern UCHAR SecondProfileKey[65];
#endif
...
#ifdef _Buffalo_
			case WSC_ID_RF_BAND:
				pProfile->Profile[CurrentIdx].Band = (*pData);
				break;
#endif
	...
	if (pWscControl->WscV2Info.bEnableWpsV2)
	{
		...
		*/
		if (CurOpMode == AP_MODE)
		{
			...
			ConfigMethods = (pWscControl->WscConfigMethods & 0x21FF); //EDX WPS icon
		}
		}
		else
#endif /* WSC_V2_SUPPORT */
...
#ifdef _Buffalo_
	UINT8 temp_int = 3;
	templen = AppendWSCTLV(WSC_ID_RF_BAND, pData, (UINT8 *)&temp_int, 0);
#else
...
/*	IV_EncrData = kmalloc(512, MEM_ALLOC_FLAG); */
...
#ifdef _Buffalo_
	AuthType=2;
	CerLen += AppendWSCTLV(WSC_ID_NW_INDEX, &TB[0], (UINT8 *)&AuthType, 0);
#else
	...
	*/
...
#if 0//def _Buffalo_
    printk("========================== <7610> pWscControl->RegData.PeerInfo.Version2 = %d\n", pWscControl->RegData.PeerInfo.Version2);
	if (pWscControl->RegData.PeerInfo.Version2 == 0)
		CerLen += AppendWSCTLV(WSC_ID_SSID, &TB[CerLen], pCredential->SSID.Ssid, 0);
	else
#endif
...
#ifdef _Buffalo_ //dual
	AuthType=2;
	CerLen += AppendWSCTLV(WSC_ID_RF_BAND, &TB[CerLen], (UINT8 *)&AuthType, 0);
	...
	AuthType = SecondProfileAuthMode;
	EncrType = SecondProfileEncrType;
	if (pWscControl->RegData.PeerInfo.Version2 == 0)
	{
		if (AuthType == (WSC_AUTHTYPE_WPAPSK | WSC_AUTHTYPE_WPA2PSK))
			AuthType = WSC_AUTHTYPE_WPA2PSK;
    ...
    AuthType = cpu2be16(AuthType);
    EncrType = cpu2be16(EncrType);
	CerLen += AppendWSCTLV(WSC_ID_SSID, &TB[CerLen], SecondProfileSsid, strlen(SecondProfileSsid));
	CerLen += AppendWSCTLV(WSC_ID_AUTH_TYPE, &TB[CerLen], (UINT8 *)&AuthType, 0);
	CerLen += AppendWSCTLV(WSC_ID_ENCR_TYPE, &TB[CerLen], (UINT8 *)&EncrType, 0);
	CerLen += AppendWSCTLV(WSC_ID_NW_KEY_INDEX, &TB[CerLen], &pCredential->KeyIndex, 0);
	CerLen += AppendWSCTLV(WSC_ID_NW_KEY, &TB[CerLen], SecondProfileKey, strlen(SecondProfileKey));
	CerLen += AppendWSCTLV(WSC_ID_MAC_ADDR, &TB[CerLen], SecondProfileMac, 0);
	AuthType=1;
	CerLen += AppendWSCTLV(WSC_ID_RF_BAND, &TB[CerLen], (UINT8 *)&AuthType, 0);
#endif
...
#ifdef _Buffalo_ //dual
		PlainLen += AppendWSCTLV(WSC_ID_CREDENTIAL, &Plain[PlainLen], TB, CerLen);
#else
		PlainLen += AppendWSCTLV(WSC_ID_CREDENTIAL, &Plain[0], TB, CerLen);
#endif
	 }
