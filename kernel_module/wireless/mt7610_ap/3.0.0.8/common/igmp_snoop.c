	...
	PSTRING buffer)
{
	...
	if(RTMPGetKeyParameter("IgmpSnEnable", tmpbuf, 128, buffer, TRUE))
	{
		...
		DBGPRINT(RT_DEBUG_OFF, (" IGMP Snooping Enable=%d\n", pAd->ApCfg.IgmpSnoopEnable));
	}
}
...
//EDX RexHua start
#if 1	// EDX, RexHua modify 
		if(1)
		{
			int reserved=0;
			PUCHAR pIpHeader = pSrcBufVA + 12;

                	if(ntohs(*((UINT16 *)(pIpHeader))) == ETH_P_IP)
			{
				struct iphdr *iph = (struct iphdr *)(pSrcBufVA + ETH_HLEN);
				if((ntohl(iph->daddr)&0xFFFFFF00)==0xE0000000) // 224.0.0.x
					reserved=1;
				if(ntohl(iph->daddr) == 0xEFFFFFFA) // 239.255.255.250
				{
					reserved=1;
					//DBGPRINT(RT_DEBUG_ERROR, ("239.255.255.250, mac=%02x:%02x:%02x:%02x:%02x:%02x %p %d hit\n", pSrcBufVA[0], pSrcBufVA[1], pSrcBufVA[2], pSrcBufVA[3], pSrcBufVA[4], pSrcBufVA[5], pAd, FromWhichBSSID));
				}
			}
			else if(ntohs(*((UINT16 *)(pIpHeader))) == ETH_P_IPV6)
			{
				struct _rt_ipv6_hdr_ *ipv6h = (struct ipv6hdr *)(pSrcBufVA + ETH_HLEN);
				if((ipv6h->dstAddr.ipv6_addr[0] == 0xff) && ((ipv6h->dstAddr.ipv6_addr[1] & 0x10) == 0)) // ff0x::x
                                        reserved=1;
                        }
                        else // non ip multicast frame
                                reserved=1;
