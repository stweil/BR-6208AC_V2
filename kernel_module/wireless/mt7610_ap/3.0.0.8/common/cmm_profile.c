...
#ifdef _Buffalo_ //dual
UCHAR SecondProfileSsid[MAX_LEN_OF_SSID+1];
UCHAR SecondProfileMac[6];
USHORT SecondProfileAuthMode;
USHORT SecondProfileEncrType;
UCHAR SecondProfileKey[65];
#endif
...
#ifdef _Buffalo_ //dual
	if(RTMPGetKeyParameter("SecondProfileSsid", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, FALSE))
	{
		NdisZeroMemory(SecondProfileSsid, sizeof(SecondProfileSsid));
		memcpy(SecondProfileSsid, tmpbuf, strlen(tmpbuf));
	}
	if(RTMPGetKeyParameter("SecondProfileMac", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE))
	{
		if( strlen(tmpbuf) == 17 && strcmp(tmpbuf,"00:00:00:00:00:00") )
		{
			macptr = tmpbuf;
			for (j=0; j<ETH_LENGTH_OF_ADDRESS; j++)
			{
				AtoH(macptr, &SecondProfileMac[j], 1);
				macptr=macptr+3;
			}
		}
	}
	if(RTMPGetKeyParameter("SecondProfileAuthMode", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE))
	{
		if ((strncmp(tmpbuf, "WPAPSKWPA2PSK", 13) == 0) || (strncmp(tmpbuf, "wpapskwpa2psk", 13) == 0))
			SecondProfileAuthMode = (WSC_AUTHTYPE_WPAPSK | WSC_AUTHTYPE_WPA2PSK);
		else if ((strncmp(tmpbuf, "WPAPSK", 6) == 0) || (strncmp(tmpbuf, "wpapsk", 6) == 0))
			SecondProfileAuthMode = WSC_AUTHTYPE_WPAPSK;
		else if ((strncmp(tmpbuf, "WPA2PSK", 7) == 0) || (strncmp(tmpbuf, "wpa2psk", 7) == 0))
			SecondProfileAuthMode = WSC_AUTHTYPE_WPA2PSK;
		else
			SecondProfileAuthMode = WSC_AUTHTYPE_OPEN;
	}
	if(RTMPGetKeyParameter("SecondProfileEncrType", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE))
	{
		if ((strncmp(tmpbuf, "WEP", 3) == 0) || (strncmp(tmpbuf, "wep", 3) == 0))
			SecondProfileEncrType = WSC_ENCRTYPE_WEP;
		else if ((strncmp(tmpbuf, "TKIPAES", 7) == 0) || (strncmp(tmpbuf, "tkipaes", 7) == 0))
			SecondProfileEncrType = (WSC_ENCRTYPE_TKIP | WSC_ENCRTYPE_AES);
		else if ((strncmp(tmpbuf, "TKIP", 4) == 0) || (strncmp(tmpbuf, "tkip", 4) == 0))
			SecondProfileEncrType = WSC_ENCRTYPE_TKIP;
		else if ((strncmp(tmpbuf, "AES", 3) == 0) || (strncmp(tmpbuf, "aes", 3) == 0))
			SecondProfileEncrType = WSC_ENCRTYPE_AES;
		else
			SecondProfileEncrType = WSC_ENCRTYPE_NONE;
	}
	if (RTMPGetKeyParameter("SecondProfileKey", tmpbuf, 512, buffer, FALSE))
	{
		NdisZeroMemory(SecondProfileKey, sizeof(SecondProfileKey));
		if( strlen(tmpbuf)>=8 && strlen(tmpbuf)<=64)
			memcpy(SecondProfileKey, tmpbuf, strlen(tmpbuf));
	}
#endif
...
#ifdef _ADV_CONTROL_ //EDX patrick
		/* AdvContral */
		if(RTMPGetKeyParameter("AdvContral", tmpbuf, 10, pBuffer, TRUE))
		{
			advControl = (UCHAR) simple_strtol(tmpbuf, 0, 10);
			printk("@@@ 7610 AdvContral=%d\n",advControl);
		}
#endif
...
#define LETTER_CASE_TRANSLATE(txt_p, card_id)			\
	...
	else if (antenna.field.RfIcType == RFIC_7610)
	{
		/* ABGN card */
		strcpy(RFIC_word, "an");
	}
