...
static BOOLEAN EdxIsolated(IN PRTMP_ADAPTER pAd, IN ULONG From, IN ULONG To)
{ 
        return ((pAd->ApCfg.MBSSID[From].EdxIsolation) ||
                (pAd->ApCfg.MBSSID[To].EdxIsolation));
}
#endif  
//EDX SJC end
...
//EDX SJC start
                	...
                	if (EdxIsolated(pAd, FromWhichBSSID, pEntry->apidx)) {
						bDirectForward = FALSE;
						bAnnounce = FALSE;
					}
				}
