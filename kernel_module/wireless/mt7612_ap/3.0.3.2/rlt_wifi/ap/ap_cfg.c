...
//EDX SJC start
#ifdef _SSIDISOLATION_  
INT     Set_SSIDIsolation_Proc(
        IN      PRTMP_ADAPTER   pAdapter,
        IN      PSTRING                 arg);
#endif  
// EDX SJC end
...
//EDX SJC start
#ifdef _SSIDISOLATION_
        {"SSIDIsolation",               Set_SSIDIsolation_Proc},
#endif 
//EDX SJC end
//EDX wade start for wps2 shortslottime switch . wps2 have to be 1 
#ifdef WSC_V2_SUPPORT
	{"ShortSlotTime",		Set_ShortSlotTime_Proc},
#endif 
//EDX wade end
	...
	IN	PSTRING			arg)
{
	...
	NoForwarding = simple_strtol(arg, 0, 10);
//#ifdef HAS_ADV_BSSID_SEPARATOR
#if 1 //EDX patrick. patch form MT7615-4402, HAS_ADV_BSSID_SEPARATOR
	if (NoForwarding == 1)
		NoForwarding = TRUE;
	else if (NoForwarding == 0)
		NoForwarding = FALSE;
	...
	if (pAd->ApCfg.MBSSID[pObj->ioctl_if].IsolateInterStaTrafficBTNBSSID != NoForwarding)
	{
		pAd->ApCfg.MBSSID[pObj->ioctl_if].IsolateInterStaTrafficBTNBSSID = NoForwarding;
	}

	DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d) Set_NoForwardingBTNSSID_Proc::(IsolateInterStaTrafficBTNBSSID=%ld)\n", pObj->ioctl_if, pAd->ApCfg.MBSSID[pObj->ioctl_if].IsolateInterStaTrafficBTNBSSID));
#else
	if (NoForwarding == 1)
		pAd->ApCfg.MBSSID[pObj->ioctl_if].IsolateInterStaTraffic = TRUE;
	else if (NoForwarding == 0)
...
//EDX start
#if defined(_E2P_ALL_) &&  defined(RTMP_FLASH_SUPPORT)
   BOOLEAN				IsWriteValuetoE2pROM = FALSE;
#endif
//EDX end
...
//EDX start
#if defined(_E2P_ALL_) &&  defined(RTMP_FLASH_SUPPORT)
				rtmp_ee_flash_memset(pAdapter, eepAddr, eepValue);	
#else 
//EDX end
...
//EDX start
#if defined(_E2P_ALL_) &&  defined(RTMP_FLASH_SUPPORT)
		if (IsWriteValuetoE2pROM) {
			rtmp_ee_flash_getmem_write(pAdapter);
		}
#endif
//EDX end
	} 
...
// EDX
VOID RTMPAPIoctlStainfoUser(
        IN      PRTMP_ADAPTER   pAdapter,
        IN      RTMP_IOCTL_INPUT_STRUCT *wrq)
{
/******/
	PSTRING mpool, msg;
	UINT32 TotalLen = 16384;

	os_alloc_mem(NULL, (UCHAR **)&mpool, TotalLen);
	NdisZeroMemory(mpool, TotalLen);
	msg = (PSTRING)((ULONG)(mpool+3) & (ULONG)~0x03);
/******/
	...
	sprintf(msg+strlen(msg),"\n");
	RTMP_IO_READ32(pAdapter, BKOFF_SLOT_CFG, &RegValue);
	sprintf(msg+strlen(msg),"BackOff Slot      : %s slot time, BKOFF_SLOT_CFG(0x1104) = 0x%08x\n", 
			OPSTATUS_TEST_FLAG(pAdapter, fOP_STATUS_SHORT_SLOT_INUSED) ? "short" : "long",
 			RegValue);

#ifdef DOT11_N_SUPPORT
	sprintf(msg+strlen(msg),"HT Operating Mode : %d\n", pAdapter->CommonCfg.AddHTInfo.AddHtInfo2.OperaionMode);
	sprintf(msg+strlen(msg),"\n");
#endif /* DOT11_N_SUPPORT */
...
#ifdef MAC_REPEATER_SUPPORT	
	MaxWcidNum = MAX_LEN_OF_MAC_TABLE + ((MAX_EXT_MAC_ADDR_SIZE + 1) * MAX_APCLI_NUM);
#endif /* MAC_REPEATER_SUPPORT */
	...
	for (i=0; i<MaxWcidNum; i++)
	{
			...
			&& (pEntry->Sst == SST_ASSOC))
		{
			...
			sprintf(msg+strlen(msg),"\t\t\t\t\t\t\t%-10s", get_phymode_str(pEntry->MaxHTPhyMode.field.MODE));
			sprintf(msg+strlen(msg),"%-6s", get_bw_str(pEntry->MaxHTPhyMode.field.BW));
#ifdef DOT11_VHT_AC
			if (pEntry->MaxHTPhyMode.field.MODE == MODE_VHT)
				sprintf(msg+strlen(msg),"%dS-M%d  ", ((pEntry->MaxHTPhyMode.field.MCS>>4) + 1), (pEntry->MaxHTPhyMode.field.MCS & 0xf));
			else
#endif /* DOT11_VHT_AC */
			sprintf(msg+strlen(msg),"%-6d", pEntry->MaxHTPhyMode.field.MCS);
			sprintf(msg+strlen(msg),"%-6d", pEntry->MaxHTPhyMode.field.ShortGI);
...
#endif /* APCLI_SUPPORT */
        {
//        	pWscControl->WscConfMode = (ConfModeIdx & WSC_ENROLLEE_PROXY_REGISTRAR); //EDX Ken , Fix.  profile WscConfMode value Forcibly change for 7
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	...
	//Profile.WscSsidLen = pMbss->SsidLen;	 //EDX Randall
	...
	if (Profile.WscEncrypType == 2)
	{
		Profile.DefaultKeyIdx = wdev->DefaultKeyId + 1;
		{
			...
			//Profile.WscWPAKeyLen = pAd->SharedKey[apidx][wdev->DefaultKeyId].KeyLen; //EDX Randall
	...
	else if (Profile.WscEncrypType >= 4)
	{
		...
		//Profile.WscWPAKeyLen = pMbss->WscControl.WpaPskLen; //EDX Randall
	...
	IN	RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	WSC_CONFIGURED_VALUE Profile;
	POS_COOKIE  pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR	    CurApIdx = pObj->ioctl_if;
	struct wifi_dev *wdev;

	PAPCLI_STRUCT pApCliTab = NULL;
	pApCliTab = &pAd->ApCfg.ApCliTab[CurApIdx];
	wdev = &pApCliTab->wdev;
	memset(&Profile, 0x00, sizeof(WSC_CONFIGURED_VALUE));

	NdisZeroMemory(Profile.WscSsid, 32 + 1);
	NdisMoveMemory(Profile.WscSsid, pApCliTab->Ssid, pApCliTab->SsidLen);
	Profile.WscSsid[pApCliTab->SsidLen] = '\0';
	//Profile.WscSsidLen = pApCliTab->SsidLen;

	Profile.WscAuthMode = WscGetAuthType(pApCliTab->wdev.AuthMode);
	Profile.WscEncrypType = WscGetEncryType(pApCliTab->wdev.WepStatus);
	
	if (Profile.WscEncrypType != WSC_ENCRTYPE_NONE)
	{
		Profile.DefaultKeyIdx = 2;
		if (Profile.WscEncrypType & (WSC_ENCRTYPE_TKIP | WSC_ENCRTYPE_AES))
		{
			//pApCliTab->DefaultKeyId = 0;
			if (pApCliTab->WscControl.WpaPskLen >= 8 && pApCliTab->WscControl.WpaPskLen <= 64)
			{
				NdisZeroMemory(Profile.WscWPAKey, 64 + 1);
				NdisMoveMemory(Profile.WscWPAKey, pApCliTab->WscControl.WpaPsk, pApCliTab->WscControl.WpaPskLen);
				Profile.WscWPAKey[pApCliTab->WscControl.WpaPskLen] = '\0';
				//Profile.WscWPAKeyLen=pApCliTab->WscControl.WpaPskLen;

				printk("%s(): WscWPAKey = \"%s\"\n",  __FUNCTION__, Profile.WscWPAKey);
				//printk("%s(): WscWPAKeyLen = %d\n",  __FUNCTION__, Profile.WscWPAKeyLen);
				DBGPRINT(RT_DEBUG_TRACE, ("WpaPskLen = %d\n", pApCliTab->WscControl.WpaPskLen));
			}
			else
			{
				DBGPRINT(RT_DEBUG_TRACE, ("WPAPSK: Invalid Key Length (%d)\n", pApCliTab->WscControl.WpaPskLen));
				pApCliTab->WscControl.WpaPskLen = 0;
			}
		}
	}
	else
        ...
        IN      RTMP_IOCTL_INPUT_STRUCT *wrq)
{
        ...
        memset(&Profile, 0x00, sizeof(ApcliGetBw));
        if (copy_from_user(&Profile, wrq->u.data.pointer, wrq->u.data.length))
        {
                DBGPRINT(RT_DEBUG_TRACE, ("%s: copy_from_user() fail\n", __FUNCTION__));
        }
printk("Profile.AP_MAC=%s\n", Profile.AP_MAC);
        ...
        for(i=0; i < MaxWcidNum; i++)
        {
                        ...
                        && (pEntry->Sst == SST_ASSOC))
                {
                        ...
                        printk("%02X:%02X:%02X:%02X:%02X:%02X\n",
                                pEntry->Addr[0], pEntry->Addr[1], pEntry->Addr[2],
                                pEntry->Addr[3], pEntry->Addr[4], pEntry->Addr[5]);
                        snprintf(temp_MAC, sizeof(temp_MAC), "%02X:%02X:%02X:%02X:%02X:%02X", pEntry->Addr[0], pEntry->Addr[1], pEntry->Addr[2], pEntry->Addr[3], pEntry->Addr[4], pEntry->Addr[5]);
                        printk("temp_MAC=%s\n", temp_MAC);
                        if(!strcmp(temp_MAC, Profile.AP_MAC))
                        {
                                printk("pEntry->HTPhyMode.field.BW=%02X\n", pEntry->HTPhyMode.field.BW);
                                //printk("GetBW(pEntry->HTPhyMode.field.BW)=%s", GetBW(pEntry->HTPhyMode.field.BW));
                                Profile.CurrentBW = (int)pEntry->HTPhyMode.field.BW;
                                printk("Profile.CurrentBW=%d\n", Profile.CurrentBW);
                                break;
                        }
                }
        }
	...
	IN PSTRING arg)
{
	...
	#if 0 //EDX patrick add. MTK patch: 01_update_mcast_vht_rate
	...
	switch (PhyMode)
	{
		...
		case MCAST_OFDM:	/* OFDM */
			...
			pAd->CommonCfg.MCastPhyMode.field.BW =  BW_20; //EDX patrick add. MTK patch: 01_update_mcast_vht_rate
			break;
#ifdef DOT11_N_SUPPORT
		case MCAST_HTMIX:	/* HTMIX */
			pAd->CommonCfg.MCastPhyMode.field.MODE = MODE_HTMIX;
			#if 1 //EDX patrick add. MTK patch: 01_update_mcast_vht_rate
        ...
        IN      PSTRING                 arg)
{
        ...
        Isolation = simple_strtol(arg, 0, 10);

        if (Isolation == 1)
                pAd->ApCfg.MBSSID[pObj->ioctl_if].EdxIsolation = TRUE;
        else if (Isolation == 0)
                pAd->ApCfg.MBSSID[pObj->ioctl_if].EdxIsolation = FALSE;
        else
                return FALSE;  //Invalid argument 
        ...
        IN      PSTRING                 arg)
{
        ...
        Shortslottime = simple_strtol(arg, 0, 10);

        if (Shortslottime == 1)
                pAd->ApCfg.MBSSID[pObj->ioctl_if].EdxShortSlotTime = TRUE;
        else if (Shortslottime == 0)
                pAd->ApCfg.MBSSID[pObj->ioctl_if].EdxShortSlotTime = FALSE;
        else
                return FALSE;  //Invalid argument 
...
#if 1 //EDX Randall
		case CMD_RTPRIV_IOCTL_GET_APCLI_PROFILE:
		    RTMPIoctlGetApCliProfile(pAd,wrq);		
		    break;

		/*case CMD_RTPRIV_IOCTL_GET_BW:
		    RTMPIoctlGetBw(pAd,wrq);
		    break;*/
#endif
...
#if 1 //edx
		case CMD_RTPRIV_IOCTL_STAINFO_USER:
			RTMPAPIoctlStainfoUser(pAd, wrq);
			break;
#endif
