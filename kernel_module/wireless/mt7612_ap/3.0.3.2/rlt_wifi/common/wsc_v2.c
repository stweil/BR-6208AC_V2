...
#include    "wsc_tlv.h"
static PinAttackTime=0; //EDX wade A band + G band time pinattack
...
//EDX wade start
VOID    Updata_Lock_Beacon(PRTMP_ADAPTER tempAd,PWSC_CTRL tempWsc)
{
                WscBuildBeaconIE(tempAd,
                                                 tempWsc->WscConfStatus,
                                                 FALSE,
                                                 0,
                                                 0,
                                                 (tempWsc->EntryIfIdx & 0xF),
                                                 NULL,
                                                 0,
                                                 AP_MODE);
                WscBuildProbeRespIE(tempAd,
                                                        WSC_MSGTYPE_AP_WLAN_MGR,
                                                        tempWsc->WscConfStatus,
                                                        FALSE,
                                                        0,
                ...
                APUpdateBeaconFrame(tempAd, tempWsc->EntryIfIdx & 0x0F);
}
VOID    Lock_AG_band_WPS(PRTMP_ADAPTER       pAd)
{
if (1)
{
struct net *net=NULL;
struct net_device *dev=NULL;
PRTMP_ADAPTER tempAd=NULL;
PWSC_CTRL tempWsc=NULL;

dev = dev_get_by_name(dev_net(pAd->net_dev), "rai0");
if (dev != NULL)
{
	tempAd = GET_PAD_FROM_NET_DEV(tempAd,dev);
	tempWsc = &tempAd->ApCfg.MBSSID[0].WscControl;
	tempWsc->bSetupLock = TRUE;
	Updata_Lock_Beacon(tempAd,tempWsc);
}
dev = dev_get_by_name(dev_net(pAd->net_dev), "ra0");
if (dev != NULL)
{
	tempAd = GET_PAD_FROM_NET_DEV(tempAd,dev);
	tempWsc = &tempAd->ApCfg.MBSSID[0].WscControl;
	...
	IN  PWSC_CTRL		pWscControl)
{
	...
	*/
	PinAttackTime ++; //EDX wade no matter the incomming pin is Aband or Gband pinattack +1
	pWscControl->PinAttackCount++;
//EDX wade total pinattack over ten time that router lock Aband and Gband
//EDX wade	if (pWscControl->PinAttackCount >= pWscControl->MaxPinAttack)
	if (PinAttackTime > pWscControl->MaxPinAttack) //EDX wade

	{		
//EDX wade start
		PinAttackTime=11;
		Lock_AG_band_WPS(pAd);
//EDX wade end	
		pWscControl->bSetupLock = TRUE;
#if 0 //EDX wade pinattack over 10 time that pin locked forever
...
#if 0 //EDX delete wade wps2 is locked that no need this
			WscBuildBeaconIE(pAd, 
							 pWscControl->WscConfStatus, 
							 FALSE,
							 0,
