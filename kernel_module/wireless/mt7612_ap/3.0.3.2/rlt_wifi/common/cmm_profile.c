...
#if 0 //def _ADV_CONTROL_ //EDX patrick
int advControl = 0;
#endif
...
#ifdef MCAST_RATE_SPECIFIC //EDX patrick add. MTK patch: 01_update_mcast_vht_rate
	...
	IN PSTRING	pBuffer)
{
	...
	/*#endif*/
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			if(RTMPGetKeyParameter("NoForwardingBTNBSSID", tmpbuf, 32, pBuffer, TRUE))
			{
				for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
				{
					int apidx = i;
					...
					if(simple_strtol(macptr, 0, 10) != 0)  //Enable
						pAd->ApCfg.MBSSID[apidx].IsolateInterStaTrafficBTNBSSID = TRUE;								
					else //Disable
						pAd->ApCfg.MBSSID[apidx].IsolateInterStaTrafficBTNBSSID = FALSE;								

					DBGPRINT(RT_DEBUG_TRACE, ("I/F(ra%d) NoForwardingBTNBSSID=%ld\n", i, pAd->ApCfg.MBSSID[apidx].IsolateInterStaTrafficBTNBSSID));
				}
			}
#else
				...
				IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
				{
					...
					{	
						...
						switch (PhyMode)
						{
								...
								break;
//#endif /* DOT11_N_SUPPORT */									
#endif /* DOT11_VHT_AC */ //EDX patrick add. MTK patch: 01_update_mcast_vht_rate
