...
void periodic_monitor_false_cca_adjust_vga(RTMP_ADAPTER *pAd)
{
	if ((pAd->CommonCfg.lna_vga_ctl.bDyncVgaEnable) && 
		(pAd->MacTab.Size > 0) && //EDX patrick add. MTK patch: 02_stop_dynamic_vga_if_no_connection
		...
		if (pAd->RalinkCounters.OneSecFalseCCACnt > pAd->CommonCfg.lna_vga_ctl.nFalseCCATh) {
			if (val1 > (pAd->CommonCfg.lna_vga_ctl.agc_vga_init_0 - 0x10)) {
				...
				bbp_val1 = (bbp_val1 & 0xffff80ff) | (val1 << 8);
				RTMP_BBP_IO_WRITE32(pAd, AGC1_R8, bbp_val1);
				#if 1 //EDX patrick add. MTK patch: 03_dynamic_cck_pd
				if ((pAd->chipCap.compensate_level == 0x10) && (pAd->CommonCfg.Channel <= 14))
                {
                    RTMP_BBP_IO_WRITE32(pAd, AGC1_R51, 0x280E3030);
                    RTMP_BBP_IO_WRITE32(pAd, AGC1_R53, 0x201C2128);
                    RTMP_BBP_IO_WRITE32(pAd, AGC1_R55, 0x50434A50);
               	}
               	#endif /* MTK patch */
#ifdef DFS_SUPPORT
            ...
            {
	            RTMP_BBP_IO_WRITE32(pAd, AGC1_R51, 0x16161C1C);
	            RTMP_BBP_IO_WRITE32(pAd, AGC1_R53, 0x26262A2F);
	            RTMP_BBP_IO_WRITE32(pAd, AGC1_R55, 0x40405466);
            }
			#endif /* MTK patch */
		}
