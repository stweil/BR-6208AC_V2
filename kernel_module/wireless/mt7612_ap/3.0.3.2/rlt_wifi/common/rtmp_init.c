...
//EDX yihong close
//#if 0  //EDX wade   read "ifconfig hw ether" mac 
/*        unsigned char zero1[6]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
        unsigned char zero2[6]={0x00,0x00,0x00,0x00,0x00,0x00};

        pAd->PermanentAddress[0] = pAd->net_dev->dev_addr[0];
        pAd->PermanentAddress[1] = pAd->net_dev->dev_addr[1];
        pAd->PermanentAddress[2] = pAd->net_dev->dev_addr[2];
        pAd->PermanentAddress[3] = pAd->net_dev->dev_addr[3];
        pAd->PermanentAddress[4] = pAd->net_dev->dev_addr[4];
        pAd->PermanentAddress[5] = pAd->net_dev->dev_addr[5];
        DBGPRINT(RT_DEBUG_TRACE, ("EDX Initialize MAC Address from ifconfig hw ether \n"));

        DBGPRINT(RT_DEBUG_TRACE, ("EDX MAC: =%02x:%02x:%02x:%02x:%02x:%02x\n",
                                                                PRINT_MAC(pAd->PermanentAddress)));

        if ((memcmp(pAd->PermanentAddress, zero1, 6) == 0) ||
            (memcmp(pAd->PermanentAddress, zero2, 6) == 0)) {
                DBGPRINT(RT_DEBUG_TRACE, ("Error can't read \"hw ether\" change to read E2PROM\n"));
...
#ifdef WSC_V2_SUPPORT
				//EDX wade config_method keypad=0 our router NO keypad onbord 
				//EDX wade pWscControl->WscConfigMethods= 0x238C; //keypad =1
				pWscControl->WscConfigMethods= 0x228C; //EDX wade keypad =0
#else
