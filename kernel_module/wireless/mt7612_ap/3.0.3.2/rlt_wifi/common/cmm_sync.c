...
CHAR ConvertToRssi(RTMP_ADAPTER *pAd, CHAR Rssi, UCHAR rssi_idx)
{
	...
	*/
	...
	if (IS_MT76x2(pAd)) {
// EDX patrick, MTK positive_RSSI_iLNA_fix patch
		CHAR extra_gain = 0;
...
// EDX patrick, MTK positive_RSSI_iLNA_fix patch
		...
		if (is_external_lna_mode(pAd, pAd->CommonCfg.Channel) == TRUE)
			extra_gain = LNAGain = 0; // EDX patrick, MTK positive_RSSI_iLNA_fix patch


		if (pAd->LatchRfRegs.Channel > 14)
			return (Rssi + pAd->ARssiOffset[rssi_idx] - extra_gain); // EDX patrick, MTK positive_RSSI_iLNA_fix patch
		else
			return (Rssi + pAd->BGRssiOffset[rssi_idx] - extra_gain); // EDX patrick, MTK positive_RSSI_iLNA_fix patch
	}
