	...
	*/
		...
		(pWpsCtrl->WscState <= WSC_STATE_WAIT_REQ_ID))
    {
		...
		DBGPRINT(RT_DEBUG_TRACE, ("WscEAPOLStartAction(ra%d) - send EAP-Req(Id) to %02x:%02x:%02x:%02x:%02x:%02x\n",
					pEntry->apidx, PRINT_MAC(pEntry->Addr)));
	...
	else if (MsgType == WSC_MSG_EAP_ENR_RSP_ID)
	{
		...
		else
		{
			...
			WscSendUPnPMessage(pAdapter, (pWscControl->EntryIfIdx & 0x0F), 
									...
									&pAdapter->ApCfg.MBSSID[pWscControl->EntryIfIdx& 0x0F].wdev.bssid[0], MAC_ADDR_LEN, 0, 0, &pAdapter->ApCfg.MBSSID[pWscControl->EntryIfIdx& 0x0F].wdev.bssid[0], AP_MODE);
#endif /* CONFIG_AP_SUPPORT */
			}
			else 
#endif /* CONFIG_AP_SUPPORT */
	...
	if (pWscControl != NULL)
	{
		...
		if (pWscControl->Wsc2MinsTimerRunning)
		{
			...
			WscSendUPnPMessage(pAd, (pWscControl->EntryIfIdx & 0x0F), 
									...
									&pAd->ApCfg.MBSSID[pWscControl->EntryIfIdx& 0x0F].wdev.bssid[0], MAC_ADDR_LEN, 0, 0, &pAd->ApCfg.MBSSID[pWscControl->EntryIfIdx& 0x0F].wdev.bssid[0], AP_MODE);
#endif /* CONFIG_AP_SUPPORT */
...
{	
	...
	if (CurOpMode == AP_MODE)
	{
		...
		NdisMoveMemory(pWscControl->RegData.SelfInfo.MacAddr, pAdapter->ApCfg.MBSSID[apidx].wdev.bssid, 6);
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	IN  UCHAR				CurOpMode)
{
	...
	if (CurOpMode == AP_MODE)
	{
		...
		MAKE_802_3_HEADER(Header802_3, 
						  &pEntry->Addr[0], 
						  &pAd->ApCfg.MBSSID[pEntry->apidx].wdev.bssid[0], 
						  EAPOL);
	}
#endif /* CONFIG_AP_SUPPORT */
    ...
    IN  UCHAR			CurOpMode)
{
	...
	if (CurOpMode == AP_MODE)
	{
		...
		snprintf((PSTRING) WepKeyName, sizeof(WepKeyName), "Key%dStr1=", pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.DefaultKeyId+1);
		snprintf((PSTRING) WepKeyFormatName, sizeof(WepKeyFormatName), "Key%dType=", pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.DefaultKeyId+1);
	}
#endif /* CONFIG_AP_SUPPORT */
