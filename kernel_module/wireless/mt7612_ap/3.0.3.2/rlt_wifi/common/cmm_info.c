	...
	IN  UINT32 MsgLen)
{
		...
		(pBss->AuthMode <= Ndis802_11AuthModeWPA1PSKWPA2PSK))
	{
				 ...
				 (pBss->AuthMode == Ndis802_11AuthModeWPA2PSK))
		{
			...
			#if 1 //edx patrick add. Fixed WPA-Mixed Mode (WPA-TKIP+WPA2-AES or WPA-AES+WPA2-TKIP) always display TKIPAES
			if (pBss->WPA.PairCipher != pBss->WPA2.PairCipher)
			{
				if(pBss->WPA.PairCipher == 4) //edx patrick add.  WPA-TKIP+WPA2-AES -> we use WPA2-AES
					ap_cipher = Ndis802_11Encryption3Enabled;
				else if(pBss->WPA.PairCipher == 6) //edx patrick add. WPA-AES+WPA2-TKIP -> we use WPA2-TKIP
					ap_cipher = Ndis802_11Encryption2Enabled;
				else
	...
	sprintf(msg+strlen(msg), "%-23s", SecurityStr);
		...
		sprintf(msg+strlen(msg),"%-6d", Rssi);
		
		
		/*printk(">>[%-3d,%-3d,%-3d,%-3d,%-3d,%-3d]\n", pBss->HtCapabilityLen, pBss->vht_cap_len, pBss->vht_op_len,
																			pBss->vht_cap_ie.vht_cap.ch_width, pBss->vht_cap_ie.vht_cap.htc_vht_cap,
																			pBss->vht_op_ie.vht_op_info.ch_width); //EDX patrick add
		*/
		...
		if (wireless_mode == Ndis802_11FH ||
			...
			sprintf(msg+strlen(msg),"%-7s", "b");
		else if (wireless_mode == Ndis802_11OFDM5)
			sprintf(msg+strlen(msg),"%-7s", "a");
		else if (wireless_mode == Ndis802_11OFDM5_N)
			sprintf(msg+strlen(msg),"%-7s", "a/n");
#ifdef DOT11_VHT_AC //EDX patrick, get AC info
		else if (wireless_mode == Ndis802_11OFDM5_N_AC)
			sprintf(msg+strlen(msg),"%-7s", "a/n/ac");
#endif
		else if (wireless_mode == Ndis802_11OFDM24)
			sprintf(msg+strlen(msg),"%-7s", "b/g");
		else if (wireless_mode == Ndis802_11OFDM24_N)
			sprintf(msg+strlen(msg),"%-7s", "b/g/n");
		else
		...
		if (pBss->AddHtInfoLen > 0)
		{
			...
			if (pBss->AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_ABOVE){
				if(pBss->vht_op_ie.vht_op_info.ch_width)
					sprintf(msg+strlen(msg),"%-7s", " 8ABOVE");
				else
					sprintf(msg+strlen(msg),"%-7s", " 4ABOVE");
			}
			else if (pBss->AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_BELOW){
				if(pBss->vht_op_ie.vht_op_info.ch_width)
					sprintf(msg+strlen(msg),"%-7s", " 8BELOW");
				else
					sprintf(msg+strlen(msg),"%-7s", " 4BELOW");
			}
			else{
...
#if 1 // Ken patch tommy+04012016 sort sitesurvey table by rssi
	PBSS_TABLE  pScanTab;
	PBSS_TABLE  pTmpScanTab;
	BSS_ENTRY	*pOutBss;
...
#if 1 // Ken patch tommy+04012016 sort sitesurvey table by rssi
	os_alloc_mem(NULL, (PUCHAR *)&pTmpScanTab, sizeof(BSS_TABLE));
	if (pTmpScanTab == NULL)
	{
		DBGPRINT(RT_DEBUG_TRACE, ("RTMPIoctlGetSiteSurvey - pTmpScanTab memory alloc fail.\n"));
		return;
	}

	pTmpScanTab->BssNr = 0;
	pTmpScanTab->BssOverlapNr = 0;
	
	for (i = 0; i < MAX_LEN_OF_BSS_TABLE; i++) 
	{
		pTmpScanTab->BssEntry[i].pVarIeFromProbRsp = NULL;
		pTmpScanTab->BssEntry[i].Rssi = -127;	/* initial the rssi as a minimum value */
	}
#endif
...
//	    "Ch", "SSID", "BSSID", "Security", "Siganl(%)", "W-Mode", " ExtCH"," NT");	
	...
	sprintf(msg+strlen(msg),"%-4s%-33s%-20s%-23s%-9s%-6s%-7s%-7s%-3s\n",
	    "Ch", "SSID", "BSSID", "Security", "Siganl(%)","Rssi", "W-Mode", " ExtCH"," NT");	
	...
	for(i=0; i<pAdapter->ScanTab.BssNr ;i++)
	{
		...
		pOutBss = &pTmpScanTab->BssEntry[i];
		NdisMoveMemory(pOutBss, pBss, sizeof(BSS_ENTRY));
		pTmpScanTab->BssNr += 1;
	}
#endif 
	BssTableSortByRssi(pTmpScanTab, FALSE);

	pScanTab = pTmpScanTab;

	for (i = 0; i < pScanTab->BssNr; i++)
	{
	...
	if (pTmpScanTab)
	{
		os_free_mem(NULL, (PUCHAR)pTmpScanTab);
	}
#endif
...
#else
#define MAC_LINE_LEN    (1+14+4+4+4+4+10+10+10+6+6+6)     /* "\n"+Addr+aid+psm+datatime+rxbyte+txbyte+current tx rate+last tx rate+"\n" */
#endif
...
/*	RT_802_11_MAC_TABLE MacTab;*/
	...
	for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	{
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC))
		{
			...
			pDst->TxRate.word = pEntry->HTPhyMode.word;
#if 1	//EDX Thomas
			pDst->BW = pEntry->HTPhyMode.field.BW;
#endif							
...
#else
	sprintf(msg+strlen(msg),"%-14s%-4s%-4s%-4s%-4s%-6s%-6s%-10s%-10s%-10s%-6s\n",
		"MAC", "AP",  "AID", "PSM", "AUTH", "CTxR", "LTxR","LDT", "RxB", "TxB", "BW");
#endif
	for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	{
