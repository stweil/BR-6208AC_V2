...
void rt_gsw_init(void)
{
	...
	u32     rev = 0;
	u32 is_BGA = 0;
#if 1	// SKU11903 EDX Randall
	RALINK_REG(RT2880_REG_PIO_0_23_DIR) |= 1 << GPIO_SWITCH_RESET;
	RALINK_REG(RT2880_REG_PIO_0_23_DATA) |= (1 << GPIO_SWITCH_RESET);
	udelay(10000); //10ms
	RALINK_REG(RT2880_REG_PIO_0_23_DATA) &= ~(1 << GPIO_SWITCH_RESET);
	udelay(10000);
	RALINK_REG(RT2880_REG_PIO_0_23_DATA) |= (1 << GPIO_SWITCH_RESET);
	...
	*(unsigned long *)(0xb0000060) &= ~(3 << 7); //set MDIO to Normal mode
	...
	mii_mgr_write(31, 0x7804, 0x1117ccf);//P5 disable
#elif defined (P5_MII_TO_MAC_MODE)
	RALINK_REG(RALINK_ETH_SW_BASE+0x3500) = 0x5e337; ////(P5, Force mode, Link Up, 100Mbps, Full-Duplex, FC ON)
	RALINK_REG(0xb0000060) &= ~(1 << 9); //set RGMII to Normal mode
...
//EDX shakim #ifdef P5_RGMII_TO_MAC_MODE
//EDX shakim start
#if defined(P4_RGMII_TO_MAC_MODE) || defined(P5_RGMII_TO_MAC_MODE)
