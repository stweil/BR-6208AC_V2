...
//EDX jerry start
	printf ("(%s %d) bd->bi_enetaddr = %02x:%02x:%02x:%02x:%02x:%02x\n", __FUNCTION__, __LINE__, bd->bi_enetaddr[0], bd->bi_enetaddr[1], bd->bi_enetaddr[2], bd->bi_enetaddr[3], bd->bi_enetaddr[4], bd->bi_enetaddr[5]); 
	printf ("(%s %d) NetOurEther = %02x:%02x:%02x:%02x:%02x:%02x\n", __FUNCTION__, __LINE__, NetOurEther[0], NetOurEther[1], NetOurEther[2], NetOurEther[3], NetOurEther[4], NetOurEther[5]);
...
//EDX jerry end
	...
	case 0:
		...
		case TFTP:
			...
			//EDX TftpStart();
//EDX start
			printf("Select server or client model: 1/2\n");
			
			if(tftpd_upg == 1){
				printf("server model start\n");
				TftpServerStart();
			}
			else{
				printf("client model start\n");
				TftpStart();
			}
//EDX end
			break;
