...
//EDX shakim start
//#ifdef SKU00002
	mii_mgr_write(0, 0, 0x3900);    //---> Disable WAN Port EDX Randall
//#endif
//EDX shakim end
//#endif
#ifdef DUAL_IMAGE_SUPPORT
...
/*config bootdelay via environment parameter: bootdelay */
...
#ifdef SKU11903
	if (1) {
		u_long value = le32_to_cpu(*(volatile u_long *)RT2880_GPIOMODE_REG);
	    value |= 1;
	    *(volatile u_long *)(RT2880_GPIOMODE_REG) = cpu_to_le32(value);
	    *(volatile u_long *)(RT2880_REG_PIO_0_23_DIR) &= ~1;
	    	
	}
	RALINK_REG(RT2880_REG_PIO_0_23_DIR) |= 1 << GPIO_POWER_LED; //output mode
	...
	// Input mode
	RALINK_REG(RT2880_REG_PIO_0_23_DIR) &= ~(1 << GPIO_RESET_BUT);
	// Output mode
	RALINK_REG(RT2880_REG_PIO_0_23_DIR) |= 1 << GPIO_WPS_BUT; //no use
	RALINK_REG(RT2880_REG_PIO_0_23_DIR) |= 1 << GPIO_APSWITCH_BUT; //no use, do not modify
	RALINK_REG(RT2880_REG_PIO_0_23_DIR) |= 1 << GPIO_POWER_LED; //output mode
	RALINK_REG(RT2880_REG_PIO_0_23_DIR) |= 1 << GPIO_WPS_LED; //output mode
	RALINK_REG(RT2880_REG_PIO_0_23_DIR) |= 1 << GPIO_2_4G_LED; //output mode
	RALINK_REG(RT2880_REG_PIO_0_23_DIR) |= 1 << GPIO_5G_LED; //output mode
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= 1 << GPIO_USB0_ENABLE; // output mode
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= 1 << GPIO_USB1_ENABLE; // output mode
	// Led on
	RALINK_REG(RT2880_REG_PIO_0_23_DATA) &= ~(1 << GPIO_POWER_LED);
	RALINK_REG(RT2880_REG_PIO_0_23_DATA) &= ~(1 << GPIO_WPS_LED);
	...
	//BSP start
#if defined (MAC_TO_REALTEK_MODE)
...
#elif defined(SKU11903)
		if((RALINK_REG(RT2880_REG_PIO_0_23_DATA) & (1 << GPIO_RESET_BUT)) == 0){
#elif defined(SKUGLP)
		if((RALINK_REG(RT2880_REG_PIO_0_23_DATA) & (1 << GPIO_RESET_BUT)) == 0){
#endif
			upgradeImage = 1;
			...
			RALINK_REG(RT2880_REG_PIO_0_23_DATA) &= ~(1 << GPIO_2_4G_LED);
			RALINK_REG(RT2880_REG_PIO_0_23_DATA) |= (1 << GPIO_5G_LED);
			RALINK_REG(RT2880_REG_PIO_0_23_DATA) |= (1 << GPIO_USB_LED);
#endif	// defined(SKU00002)
			break;
		}else{
			upgradeImage = 0;
		}
...
#if 0	// SJC
			RALINK_REG(RT2880_REG_PIO_0_23_DATA) &= ~(1 << GPIO_2_4G_LED);	//EDX wireless len on
#endif
...
//EDX shakim end
...
#if 0	// SJC
				RALINK_REG(RT2880_REG_PIO_0_23_DATA) |= (1 << GPIO_2_4G_LED);	// wireless len off
#endif
				free (head);
				//reset
				...
				for ( ; ; ){		
#if 0	// SJC
					RALINK_REG(RT2880_REG_PIO_0_23_DATA) ^= (1 << GPIO_2_4G_LED); //led on <==> off
#endif
					udelay (1000000); //1sec
				}
			}
