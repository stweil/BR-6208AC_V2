...
const char boot_string[] = "UBoot Version:"RALINK_LOCAL_VERSION"."MINOR_VERSION;		//EDX jerry
...
#ifdef SKU7478 //edx patrick
  u32 tDDR;
  tDDR = RALINK_REG(DDR_CFG0_REG);
  printf("\n\nold DDR_CFG0_REG =0x%08X\n", tDDR);
  tDDR &= ~(1<<23); // bit27:23 = Active to Pre-charge time
  RALINK_REG(DDR_CFG0_REG) = tDDR;
	printf("new DDR_CFG0_REG =0x%08X\n\n", RALINK_REG(DDR_CFG0_REG));

	#if 0
	tDDR = RALINK_REG(RALINK_MEMCTRL_BASE+0x44);
	printf("\n\nold DDR_CFG1_REG =0x%08X\n", tDDR);
	tDDR &= 0xFFFFFFF0;
  tDDR |= 0x3; // bit0:3 = RAS# to CAS# delay time
  RALINK_REG(RALINK_MEMCTRL_BASE+0x44) = tDDR;
	printf("new DDR_CFG1_REG =0x%08X\n\n", RALINK_REG(RALINK_MEMCTRL_BASE+0x44));
	#endif
	printf("new DDR_CFG1_REG =0x%08X\n\n", RALINK_REG(RALINK_MEMCTRL_BASE+0x44));
#endif
...
#define SEL_LOAD_BOOT_TFTP_SERVER       5	//EDX shakim
...
void OperationSelect(void)
{
	...
	printf("   %d: Load ucos code to SDRAM via TFTP Client. \n", SEL_LOAD_BOOT_TFTP_SERVER);	//EDX Kyle
...
int tftp_config(int type, char *argv[])
{
	...
	printf("(%s) ", devip);
//EDX start
	if (type == SEL_LOAD_BOOT_TFTP_SERVER) {
		sprintf(devip,"%s","192.168.1.6");
	} else
//EDX end
		input_value(devip);
	setenv("ipaddr", devip);
...
//EDX	else if (type == SEL_LOAD_LINUX_SDRAM ) {
	else if (type == SEL_LOAD_LINUX_SDRAM || type == SEL_LOAD_BOOT_TFTP_SERVER ) { //EDX
	...
	printf("(%s) ", file);
	if (type != SEL_LOAD_BOOT_TFTP_SERVER) //EDX	
		input_value(file);
	if (file == NULL)
		return 1;
...
//EDX Kyle start
	...
	#ifdef SKU7478 //edx patrick
  u32 tDDR;
  tDDR = RALINK_REG(RT2880_GPIOMODE_REG);
  printf("\n\nold GPIOMODE_REG =0x%08X\n", tDDR);
	#endif

	printf("RALINK_GPIOMODE_UARTF=0x%08X\n", RALINK_GPIOMODE_UARTF);
	#ifdef SKU7478
	//RALINK_REG(RT2880_GPIOMODE_REG) &= 0x0;  //set GPIO mode
	...
	#ifdef SKU7478 //edx patrick
  tDDR = RALINK_REG(RT2880_GPIOMODE_REG);
  printf("new GPIOMODE_REG =0x%08X\n", tDDR);
	#endif

#if defined (SKU6278)
	// Input mode
	RALINK_REG(RT2880_REG_PIO_0_23_DIR) &= ~(1 << GPIO_RESET_BUTTON); 

	// Output mode
	RALINK_REG(RT2880_REG_PIO_24_39_DIR) |= (1 << GPIO_LED_PORT0);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= (1 << GPIO_LED_PORT1);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= (1 << GPIO_LED_PORT2);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= (1 << GPIO_LED_PORT3);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= (1 << GPIO_LED_PORT4);

	// Led off
	RALINK_REG(RT2880_REG_PIO_24_39_DATA) |= (1 << GPIO_LED_PORT0);
	RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_LED_PORT1);
	RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_LED_PORT2);
	RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_LED_PORT3);
	RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_LED_PORT4);
#elif defined (SKU7478)

	// Input mode
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) &= ~(1 << GPIO_APSWITCH_BUT_1);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) &= ~(1 << GPIO_APSWITCH_BUT_2);
	//RALINK_REG(RT2880_REG_PIO_40_71_DIR) &= ~(1 << GPIO_WPS_BUT);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) &= ~(1 << GPIO_RESET_BUT); 

	// Output mode
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= (1 << GPIO_POWER_LED);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= (1 << GPIO_2_4G_LED);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= (1 << GPIO_5G_LED);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= (1 << GPIO_WIRELESS_LED);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= (1 << GPIO_WPS_LED);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) |= (1 << GPIO_LAN_LED);

	// Led on
	RALINK_REG(RT2880_REG_PIO_40_71_DATA) &= ~(1 << GPIO_POWER_LED);

	// Led off
	RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_2_4G_LED);
	RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_5G_LED);
	RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_WIRELESS_LED);
	RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_WPS_LED);
	RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_LAN_LED);

#else

	//config these pins to gpio mode
	// Input mode
	RALINK_REG(RT2880_REG_PIO_24_39_DIR) &= ~(1 << GPIO_APSWITCH_BUT_1);
	RALINK_REG(RT2880_REG_PIO_40_71_DIR) &= ~(1 << GPIO_APSWITCH_BUT_2);
	RALINK_REG(RT2880_REG_PIO_24_39_DIR) &= ~(1 << GPIO_WPS_BUT);
	RALINK_REG(RT2880_REG_PIO_24_39_DIR) &= ~(1 << GPIO_RESET_BUT); //no used
	// Output mode
	RALINK_REG(RT2880_REG_PIO_24_39_DIR) |= (1 << GPIO_POWER_LED);
	RALINK_REG(RT2880_REG_PIO_24_39_DIR) |= (1 << GPIO_2_4G_LED);
	RALINK_REG(RT2880_REG_PIO_24_39_DIR) |= (1 << GPIO_5G_LED);
	RALINK_REG(RT2880_REG_PIO_24_39_DIR) |= (1 << GPIO_WIRELESS_LED);
	RALINK_REG(RT2880_REG_PIO_24_39_DIR) |= (1 << GPIO_WPS_LED);
	RALINK_REG(RT2880_REG_PIO_24_39_DIR) |= (1 << GPIO_LAN_LED);
	// Led on
	RALINK_REG(RT2880_REG_PIO_24_39_DATA) &= ~(1 << GPIO_POWER_LED);
	// Led off
	RALINK_REG(RT2880_REG_PIO_24_39_DATA) |= (1 << GPIO_2_4G_LED);
	RALINK_REG(RT2880_REG_PIO_24_39_DATA) |= (1 << GPIO_5G_LED);
	RALINK_REG(RT2880_REG_PIO_24_39_DATA) |= (1 << GPIO_WIRELESS_LED);
	RALINK_REG(RT2880_REG_PIO_24_39_DATA) |= (1 << GPIO_WPS_LED);
	RALINK_REG(RT2880_REG_PIO_24_39_DATA) |= (1 << GPIO_LAN_LED);

#endif
	printf("************** Init GPIO Pin Done **************\n");
//EDX Kyle end
	//debug("\n  New gd=%08X\n",gd);
...
//EDX shakim start
#ifdef SKU00020
	mii_mgr_write(4, 0, 0x3900);    //---> Disable WAN Port
#endif
//EDX shakim end
#ifdef DUAL_IMAGE_SUPPORT
...
/*config bootdelay via environment parameter: bootdelay */
	...
	//RALINK_REG(RT2880_REG_PIO_0_23_DATA) &= ~ (1 << GPIO_POWER_LED);	//EDX Power led ON
...
#ifdef SKU6278
if((RALINK_REG(RT2880_REG_PIO_0_23_DATA) & (1 << GPIO_RESET_BUTTON)) == 0){			
			upgradeImage = 1;
			// Internet Led on		
			RALINK_REG(RT2880_REG_PIO_24_39_DATA) &= ~(1 << GPIO_LED_PORT0);
...
#endif
#ifdef SKU00020
		if((RALINK_REG(RT2880_REG_PIO_24_39_DATA) & (1 << GPIO_RESET_BUT)) == 0){		
			upgradeImage = 1;
			//Power Led on and 2_4G Led on			
			RALINK_REG(RT2880_REG_PIO_0_23_DATA) &= ~(1 << GPIO_POWER_LED);
			RALINK_REG(RT2880_REG_PIO_0_23_DATA) &= ~(1 << GPIO_2_4G_LED);
			RALINK_REG(RT2880_REG_PIO_0_23_DATA) |= (1 << GPIO_5G_LED);
#endif
#ifdef SKU7478
		if((RALINK_REG(RT2880_REG_PIO_40_71_DATA) & (1 << GPIO_RESET_BUT)) == 0){		
			upgradeImage = 1;
			//Power Led on and LAN Led on			
			RALINK_REG(RT2880_REG_PIO_40_71_DATA) &= ~(1 << GPIO_POWER_LED);
			RALINK_REG(RT2880_REG_PIO_40_71_DATA) &= ~(1 << GPIO_LAN_LED);
#endif
...
//EDX Kyle start
	tftpd_upg = upgradeImage;
	if(upgradeImage == 1){
		BootType='5'; //EDX shakim
		upgradeImage=0;
	}
//EDX Kyle end
	if(BootType == '3') {
...
//EDX shakim start
		...
		unsigned short checksume;
		unsigned long flash_content_size;
		flash_content_size = CFG_BOOTLOADER_SIZE + CFG_CONFIG_SIZE + CFG_FACTORY_SIZE + CFG_BOARD_CONFIG_SIZE;
//EDX shakim end
...
//EDX start
			int skipLen = 0;
			head=(IMG_HEADER_T *)malloc(20);
			file_addr = simple_strtoul(argv[1], NULL, 16);
			memcpy(head, file_addr, 20);
			if(!strcmp(head->signature,"CSYS")){
				NetBootFileXferSize = head->len + sizeof(IMG_HEADER_T);
			}else if(!strcmp(head->signature,"RSDK")){
				NetBootFileXferSize = head->len;
				skipLen = sizeof(IMG_HEADER_T);
			}
			free (head);
//EDX end
			...
			//EDX argv[0]= "cp.linux";
			//EDX start
			argv[0]= "cp.linux2"; //EDX
			file_addr += skipLen; //EDX
			argc = 5;
			sprintf(argv[1],"0x%08X",file_addr);
			//EDX end
...
//EDX Kyle start
		case '5':
			printf("   \n%d: System Load Boot Loader | F/W  then write to Flash via TFTP Client. \n", SEL_LOAD_BOOT_TFTP_SERVER);
			printf(" Warning!! Erase Linux in Flash then burn new one. Are you sure?(Y/N)\n");

			head=(IMG_HEADER_T *)malloc(20);
			tftp_config(SEL_LOAD_BOOT_TFTP_SERVER, argv);
			argc= 3;
      ...
      #ifdef SKU00020
			RALINK_REG(RT2880_REG_PIO_24_39_DATA) &= ~(1 << GPIO_2_4G_LED);	//EDX wireless len on
      #endif            
			...
			puts("\n****************Parse Head**************\n");
			printf("signature=%4s startAddr=0x%08x burnAddr=0x%08x nmodTag=%c%c%c%c len=0x%08x \n",
				head->signature,head->startAddr, head->burnAddr,head->modTag[0],head->modTag[1],
				head->modTag[2],head->modTag[3],head->len);
			
			puts("\n****************CheckSume**************\n");
			checksume=calculateChecksum(( char *)(file_addr+20),head->len);
			if(checksume==0){ 
				puts("CheckSum ok!");
				if(strcmp(head->signature,"BOOT")==0){
#if defined (CFG_ENV_IS_IN_SPI)
					puts("\n****************Start Upgrade Boot**************\n");
					unsigned int load_address = simple_strtoul(argv[1], NULL, 16) + 20;	//+20 bytes to skip header 
				...
				}else if(strcmp(head->signature,"CSYS")==0){
					puts("\n****************Start Upgrade Kernel**************\n");

					NetBootFileXferSize = head->len + sizeof(IMG_HEADER_T);

					if (NetBootFileXferSize <= (bd->bi_flashsize - flash_content_size)) {
#if defined (CFG_ENV_IS_IN_SPI)
						unsigned int load_address = simple_strtoul(argv[1], NULL, 16);
		                                raspi_erase_write((u8 *)load_address, CFG_KERN_ADDR - CFG_FLASH_BASE, NetBootFileXferSize);
					}else{
					...
					argv[0] = "cp.linux";
					argc = 5;
					sprintf(addr_str, "%X", CFG_KERN_ADDR);
					argv[1] = addr_str;
					do_mem_cp(cmdtp, 0, argc, argv);
					...
					do_bootm(cmdtp, 0, argc, argv);
//EDX
				}else if(strcmp(head->signature,"RSDK")==0){
					puts("\n****************Start Upgrade Kernel**************\n");

					NetBootFileXferSize = head->len;  //EDX

					if (NetBootFileXferSize <= (bd->bi_flashsize - flash_content_size)) {
#if defined (CFG_ENV_IS_IN_SPI)
						unsigned int load_address = simple_strtoul(argv[1], NULL, 16);
						load_address += sizeof(IMG_HEADER_T); //EDX
		                                raspi_erase_write((u8 *)load_address, CFG_KERN_ADDR - CFG_FLASH_BASE, NetBootFileXferSize);
					}else{
						printf("***********************************\n");
						printf("The Linux Image size is too big !! \n");
					...
					file_addr += 20;
					argc = 5;
					sprintf(argv[1],"0x%08X",file_addr); //EDX
					do_mem_cp(cmdtp, 0, argc, argv);
...
//EDX
//EDX shakim start
				}else if(strcmp(head->signature,"ANFG")==0){
					puts("\n****************Start Upgrade Kernel**************\n");
					printf ("Automatic boot of image at addr 0x%08lX ...\n",	load_addr);
					do_bootm (cmdtp, 0, 1,  argv[0]);
//EDX shakim end
				}else{

					puts("\n****************Signature ERR **************\n");
					printf("Can not identify signature ID %s:\n", head->signature);
					puts("Signature ID must be CSYS, RSDK, ANFG or BOOT!");
					break;
				}
        #ifdef SKU00020
				RALINK_REG(RT2880_REG_PIO_24_39_DATA) |= (1 << GPIO_2_4G_LED);	// wireless len off
        #endif
				
				#ifdef SKU7478
				//RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_2_4G_LED);	// wireless len off
				//RALINK_REG(RT2880_REG_PIO_40_71_DATA) |= (1 << GPIO_5G_LED);	// wireless len off
        #endif
				free (head);
				//reset
				...
				free(head);
				printf("\n Check sume err: sum=%08x\n",checksume);

				for ( ; ; ){		
          #ifdef SKU00020
					RALINK_REG(RT2880_REG_PIO_24_39_DATA) ^= (1 << GPIO_2_4G_LED); //led on <==> off
          #endif
					#ifdef SKU7478
					RALINK_REG(RT2880_REG_PIO_40_71_DATA) ^= (1 << GPIO_2_4G_LED); //led on <==> off
					RALINK_REG(RT2880_REG_PIO_40_71_DATA) ^= (1 << GPIO_5G_LED);   //led on <==> off
					//RALINK_REG(RT2880_REG_PIO_40_71_DATA) ^= (1 << GPIO_LAN_LED);   //led on <==> off
          #endif
					udelay (1000000); //1sec
				}
			}
