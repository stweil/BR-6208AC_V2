...
{
	 memmove(pMD5_CTX->HashValue, MD5_DefaultHashValue, sizeof(MD5_DefaultHashValue));
...
void MD5_Hash ( MD5_CTX_STRUC *pMD5_CTX )
{
    ...
    for (X_i = 0; X_i < 16; X_i++)
        X[X_i] = (UINT32)(X[X_i]); /* Endian Swap */
        /* End of for */
...
void MD5_Append ( MD5_CTX_STRUC *pMD5_CTX, const UINT8 Message[], UINT MessageLen )
{
    ...
    while (appendLen != MessageLen) {
        ...
        if ((pMD5_CTX->BlockLen + diffLen) < MD5_BLOCK_SIZE) {
        	   memmove(pMD5_CTX->Block+pMD5_CTX->BlockLen,Message + appendLen,diffLen);
            pMD5_CTX->BlockLen += diffLen;
            appendLen += diffLen;
        } 
        else
        {
        	   memmove(pMD5_CTX->Block+pMD5_CTX->BlockLen,Message + appendLen,MD5_BLOCK_SIZE - pMD5_CTX->BlockLen);
...
void MD5_End ( MD5_CTX_STRUC *pMD5_CTX, UINT8 DigestMessage[])
{
    ...
    memset(pMD5_CTX->Block + pMD5_CTX->BlockLen, 0x80, 1);
...
*/
    ...
    for (index = 0; index < 4;index++)
        pMD5_CTX->HashValue[index] = (UINT64)(pMD5_CTX->HashValue[index]);
