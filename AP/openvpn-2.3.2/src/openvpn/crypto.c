...
struct nlmsghdr *netlink_nlh = NULL;
static struct iovec netlink_iov;
int netlink_sock=0;
static struct msghdr netlink_msg;

#define OPENVPN_NETLINK_CRYPTO 26
...
static int send_netlink_msg(unsigned char *data, unsigned int len, unsigned short flag)
{
...
static int recv_netlink_msg(unsigned char *dst_buff, unsigned int *len)
{
	/* Fill the netlink message header */
	memset(netlink_nlh, 0, NLMSG_SPACE(MAX_OPENVPN_PAYLOAD));
	memset(&netlink_iov, 0, sizeof(netlink_iov));
	...
	recvmsg(netlink_sock, &netlink_msg, 0);
...
#ifdef CONFIG_RTK_OPENVPN_HW_CRYPTO
	   /*send data to driver */
	 send_netlink_msg(BPTR (buf), BLEN (buf), DATA_TO_ENC_FLAG);
	 recv_netlink_msg(BPTR(&work), &work.len);
...
#ifdef CONFIG_RTK_OPENVPN_HW_CRYPTO
	 /*send data to driver */
	send_netlink_msg(BPTR (buf), BLEN (buf), DATA_TO_DEC_FLAG);
#endif
      /* Verify the HMAC */
      if (ctx->hmac)
	{
	      ...
	      const char *prefix)
{
...
#ifdef CONFIG_RTK_OPENVPN_HW_CRYPTO
	  /*send cipher key to driver */
	 send_netlink_msg(key->cipher, kt->cipher_length, enc);
#endif	 
    }
  if (kt->digest && kt->hmac_length > 0)
	...
	/*send cipher key to driver */
	if(enc==DEC_KEY_FLAG)
		send_netlink_msg(key->hmac, kt->hmac_length, HMAC_DEC_KEY_FLAG);
	else
		send_netlink_msg(key->hmac, kt->hmac_length, HMAC_ENC_KEY_FLAG);
#endif
    }
  gc_free (&gc);
}
