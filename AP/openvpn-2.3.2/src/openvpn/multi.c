  ...
  if (m->hash)
      ...
      if (version == 1) /* WAS: m->status_file_version */
	{
	  ...
	  hash_iterator_free (&hi);
		...
		//EDX Robert Add
		FILE *fp;
		fp=fopen("/tmp/openvpn/account_connection_status","w");
		//
	  ...
	  while ((he = hash_iterator_next (&hi)))
	      ...
	      if (multi_route_defined (m, route))
		{
		  ...
		  status_printf (so, "%s%s,%s,%s,%s",
			...
			//EDX Robert Add
			fprintf(fp,"[%s],%s%s\n",tls_common_name (mi->context.c2.tls_multi, false),mroute_addr_print (ma, &gc),flags);
			//
		}
	      gc_free (&gc);
	    }
	  ...
	  status_printf (so, "END");
		...
		//EDX Robert Add
		fclose(fp);
		system("scriptlib_util.sh check_openvpn_account");
		//
	}
