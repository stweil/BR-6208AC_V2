...
#include "libbb.h"
unsigned start_time2; // EDX
//#define FILE_SIZE 2622200 // EDX
#define FILE_SIZE 1549820 // EDX
...
static int pump_data_and_QUIT(int from, int to)
{
	...
	//printf("==-%10d\n",(int)monotonic_sec());
	//fprintf(stderr, "Now=%d, Total Time=%d, Size=%d\n", monotonic_sec(), monotonic_sec()-start_time2, FILE_SIZE); // EDX
	//fprintf(stderr, "Bandwidth=%d\n", FILE_SIZE/(monotonic_sec()-start_time2)/1024); // EDX(KBps)
	fprintf(stderr, "Bandwidth=%d\n", FILE_SIZE/(monotonic_sec()-start_time2)/128); // EDX(Kbps)
