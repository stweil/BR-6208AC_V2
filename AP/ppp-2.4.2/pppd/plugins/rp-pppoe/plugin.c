...
"$Id: plugin.c,v 1.2 2004/10/21 09:51:40 ysc Exp $";
...
PPPOEDisconnectDevice(void)
{
    ...
    #ifdef _PPPOE_AUTO_PADT_

    if (1) {
	u_short	sess = conn->session;
	u_short	type = conn->cookie.type;
...
PPPoEDevnameHook(char *cmd, char **argv, int doit)
{
    ...
    //mark  by sc_yang to support wlan pppoe
#if 0
