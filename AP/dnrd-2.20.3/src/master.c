...
int always_fake = 0;
#endif
int master_lookup(unsigned char *msg, int len, const struct sockaddr_in *daddr) // EDX RexHua
//int master_lookup(unsigned char *msg, int len)
{
	...
	char queryname_buf[256];
	snprintf_cname(msg, len, 12, queryname_buf, sizeof(queryname_buf)); // EDX patrick Add, get query name

	//if(no_srvlist(d->srvlist))
  /*if( (no_srvlist(d->srvlist)) || (!strcmp("edimax.setup", queryname_buf)) ) // EDX patrick modify, if no server or "edimax.setup"
    if (query.type == DNS_TYPE_A) 
	if ((rec = name_lookup(query.name)) == NULL)
	    add_nameip(query.name, sizeof("query.name"), inet_ntoa(daddr->sin_addr));	// EDX RexHua
	else
	    create_nameip(&rec->u.nameip, sizeof("query.name"), inet_ntoa(daddr->sin_addr));*/
		#if defined(_WIFI_REPEATER_)
		if( (no_srvlist(d->srvlist)) || (!strcmp("edimaxext.setup", queryname_buf)) ) // EDX patrick modify, if no server or "edimaxext.setup"
		#else
		if( (no_srvlist(d->srvlist)) || (!strcmp("edimax.setup", queryname_buf)) ) // EDX patrick modify, if no server or "edimax.setup"
		#endif
		#if defined(_IQv2_)
		...
		{
			if (always_fake == 1)
				add_nameip(query.name, sizeof("query.name"), "1.2.3.4");
