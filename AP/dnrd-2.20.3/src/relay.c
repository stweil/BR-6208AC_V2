...
int handle_query(const struct sockaddr_in *fromaddrp, char *msg, int *len, domnode_t **dptr, const struct sockaddr_in *daddr)
{
    int       replylen;
    domnode_t *d;
    ...
    if ((replylen = master_lookup(msg, *len, daddr)) > 0) { // EDX RexHua
