...
var wpsConType = <%getVar("wpsConfigType","","");%>;

var methodVal5g = <%getiNICVar("encrypt","","");%>;
var methodVal2g = <%getVar("encrypt","","");%>;

var wpaMode = <%getVar("wpaCipher","","");%>
var enable1x = <%getVar("enable1X","","");%>;
...
var wlanAcEnable = <%getVar("wlanAcEnabled","","");%>
var wlsw = <%getVar("wlanDisabled","","");%>;
var wpsenable = <%getVar("wpsEnable","","");%>;

var ssidTbl2g=<%getInfo("ssidAll");%>;
var ssidTbl5g=<%getiNICInfo("ssidAll");%>;
if(useCrossBand == "1") ssidname = ssidTbl5g[0];
else ssidname = ssidTbl2g[0];

ssidname = ssidname.replace(/\s/g, "&nbsp;");

if(wizMode==2){
	if(useCrossBand == "1") wps_key="<%getiNICInfo("rep_wps_key_val");%>";
	else wps_key="<%getInfo("rep_wps_key_val");%>";
}
else
	wps_key="<%getInfo("wps_key_val");%>";
...
function savechange(rebootFlag)
{
	if(document.Wpsseting.wpsEnable.checked==false)
		document.Wpsseting.wps2g.value="OFF"
	else
		document.Wpsseting.wps2g.value="ON"
...
function saveChanges(mode)
{
  ...
  {
		configMode=confMode.selectedIndex; // 0:Registrar  1:Enrollee
		if(configMode!=1)
		{
			if(mode == 1)   //PIN
			{
...
function init()
{
   ...
   wps_key24g=wps_key.substring(0,50)+'<br>'+wps_key.substring(50,wps_key.length);
	else
	 wps_key24g=wps_key;
	document.getElementById('wpskey').innerHTML = '<div class="mini2len" ><span class="choose-itemText">'+wps_key24g+'</span></div>';
...
<body onLoad="init();">
	...
	<div class="help-information" id="help-box" style="display:none">
		<table class="help-table1">
			<tr><td><div class="help-text" id="help-info1"><script>dw(WPShelpInfor)</script>
			</div></td></tr>
		</table>
	</div>
	<script>
...
<blockquote>
	<fieldset name="dynamicip_fieldset" id="dynamicip_fieldset">
		<legend><script>if(wizMode==2)document.write('2.4'+showText(GHz)+'&nbsp;');dw(WPS)</script></legend>
			<br>
				<form action="/goform/formWpsEnable" method="POST" name="Wpsseting">
					<table align="center" border="0" Cellspacing="4" width="700">
						<tr>
							<td colspan="0" style="text-align:left; padding-left:50px;">
								<input type="checkbox" name="wpsEnable"  <%getVar("wpsEnable","1","checked");%> value="ON" onClick="savechange(0)"><span class="itemText"><script>dw(enable)</script>&ensp;<script>dw(WPS)</script></span>
									<script>
								...
								<input type="hidden" value="/wireless_wps.asp" name="wlan-url">
								<input type="hidden" value="" name="isApply">
								<input type="hidden" value="" name="wps2g">
						</tr>							
					</table>
				</form>
					...
					<table align="center" border="0" Cellspacing="4" width="700">
							...
							<tr>
								...
								<td><div class="status-itemText2-len"><span class="choose-itemText ">
							...
							<tr>
								...
								<td><div class="status-itemText2-len"><span class="choose-itemText "><%getInfo("pinNumber");%></span></div></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<script>//.getElementById('wpskey').innerHTML = '<span class="choose-itemText">'+wps_key+'</span>';</script>
					...
					<form action="/goform/formWpsStart" method="POST" name="WpsDevice">
							...
							<input type="hidden" value="0" name="band_select">
							<input type="hidden" value="" name="PinCode">
							<table align="center" border="0" Cellspacing="4" width="700">
								<tr>
									<td><div class="status-itemText-len"><b><span class="itemText"><script>dw(DeviceConfigure)</script>&ensp;:</span></b></div></td>
				...
				<table align="center" border="0" cellpadding="4" width="700">
						<tr>
							<td style="text-align:center; padding-top:15px;">
								<script>
									//document.write('<input type="button" value="'+showText(save)+' '+showText(Settings)+'" name="save" id="save" onclick="return savechange(0)" class="ui-button">');
