...
var vlantagTbl=<%getiNICInfo("vlanidAll");%>;
...
var wepkeyLenTbl =		<%getiNICInfo("wepAll");%>;
...
var rsIpTbl=<%getiNICInfo("rsIpall");%>;
var rsPortTbl=<%getiNICInfo("rsPortall");%>;
var rsPasswordTbl=<%getiNICInfo("rsPasswordall");%>;
...
var mssidindex=0;
var selectIndex=0;
var hiddenSSIDtbl = <%getiNICInfo("hiddenSSIDAll");%>;
...
var macTbl = new Array('<% getInfo("inicmac1"); %>','<% getInfo("inicmac2"); %>','<% getInfo("inicmac3"); %>','<% getInfo("inicmac4"); %>');
...
function saveChanges(rebootFlag)
{
	...
	with(document.wlanSetup) {
		...
		{
			if(typeof(_INIC_ENVLAN_) != 'undefined' && apMode==0 )
			{
...
function disableall(flagvalue)
{
	with(document.wlanSetup) 
  {
		...
		//enableMSSID.disabled = flagvalue;
			...
			<table width="700" cellpadding="2" cellspacing="2" align="center" border="0" class="footBorder">
				...
				for (i=1 ; i<5 ; i++)
				{
					...
					/*if(ssidenableTbl[i-1] == "1")
						document.write('<td style="width:85px"><input type="checkbox" name="MSSID'+i+'" checked disabled></td>');
					else
						document.write('<td style="width:85px"><input type="checkbox" name="MSSID'+i+'" disabled></td>');*/
...
<!--================================================  wlEncrypt  ==============================================================================-->
	...
	<fieldset name="wlEncrypt_fieldset" id="wlEncrypt_fieldset">
		<legend><script>dw(MultipleSSID)</script>&ensp;<script>dw(Security)</script></legend>
					...
					<input type="hidden" value="/mssid_wireless5g_basic.asp" name="submit-url">
