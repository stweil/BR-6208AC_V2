...
/* $Id: get.c,v 1.1 2010/02/09 13:53:03 bradhuang Exp $*/
...
extern void handle_hnap_GethandleScript(request *req);
...
int init_get2(request * req)
{
	...
	if(strstr(req->request_uri,".asp")==NULL)
	{		
#if defined(SUPPORT_HNAP)			
		if ((strstr(req->request_uri, "/HNAP") ==NULL)) {
		return 1;
	}
#else
			return 1;
		...
		while(1)
		{
			...
			if((left!=NULL) && (right!=NULL))
			{
				...
				while((bob+req->buffer_end+10)>(req->max_buffer_size))    //Brad modify
				{
					...
					if(ret==FAILED) {bob=req->max_buffer_size- req->buffer_end;printf("will break\n"); break;}
				}
#endif				
...
process_hanp:
	handle_hnap_GethandleScript(req);
	return 0;
#endif
...
// davidhsu  -----------------------------
		...
		if (!strcmp(req->request_uri, WEB_PAGE_LOGOUT)) {
			char tmpbuf[200];
			if (is_valid_user(req)) {
				#include "apform.h"
				char message[200];
				sprintf(message,"%s logout", req->remote_ip_addr); 
				LOG_MSG(message);	
				delete_user(req);
			}
			sprintf(tmpbuf, "/%s", directory_index);
			return websRedirect(req, tmpbuf);
		}	
#endif		
//----------------------------------------
...
//fprintf(stderr, "\r\n [%s-%u]",__FILE__,__LINE__); // RexHua


//fprintf(stderr, "\r\n [%p]",req->ranges); // RexHua
//fprintf(stderr, "\r\n [%d-%d]",req->ranges->stop, req->ranges->start); // RexHua
