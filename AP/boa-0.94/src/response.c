...
/* $Id: response.c,v 1.2 2010/03/08 10:34:01 bradhuang Exp $*/
...
void print_transfer_encoding(request * req)
{
    char * mime_type = get_mime_type(req->request_uri);

    if (mime_type != NULL) {
        req_write(req, "Transfer-Encoding: ");
        req_write(req, "Chunked");
        req_write(req, CRLF);
    }
}
...
//fprintf(stderr,"\r\n req->request_uri=%s",req->request_uri);

	if (mime_type != NULL) 
	{
	        req_write(req, "Content-Type: ");

	        if(strcmp(req->request_uri,"/version.txt")==0 || strcmp(req->request_uri,"/chklst.txt")==0)
	        {
	        	req_write(req, "text/plain");        
	        }/* below page ought to response "text/xml", otherwise the dynamic page will not be workable. Keith */
		else if(strstr(req->request_uri, "active_sessions.asp") || strstr(req->request_uri, "cgi_ssi_igmp_group_memberships.asp") ||
				...
				strstr(req->request_uri, "wifisc_cancel_add_sta.asp") || strstr(req->request_uri, "wifisc_pin.asp") || strstr(req->request_uri, "dyn_clients_only.asp") ||
				strstr(req->request_uri, "dfw_query_result.asp") || strstr(req->request_uri,"dpack_query_result.asp")
		)
		{
//fprintf(stderr,"\r\n req->request_uri=%s",req->request_uri);		
//fprintf(stderr,"\r\n Content-Type=text/xml");	
			req_write(req, "text/xml"); 
		}
	        else
	        {
	...
	// davidhsu -----------------
		if (!strcmp(mime_type, "application/configuration") && strcmp(req->request_uri,"/version.txt") != 0 && strcmp(req->request_uri,"/chklst.txt") != 0) 
		{
...
void send_r_request_ok_xml(request * req)
{
    ...
    print_http_headers(req);
		...
		print_content_type(req);
		print_transfer_encoding(req);
		req_write(req, CRLF);

}

/* R_REQUEST_OK: 200 */
extern int get_reboot_close;
...
#endif
	char e_s[MAX_HEADER_LENGTH * 3];
	char msgbuf[250], urlbuf[200], *redirectFmt;
	req->response_status = R_MOVED_PERM;
...
//		char *msgbuf, urlbuf[200], *redirectFmt;
...
//#endif
		...
		sprintf(urlbuf, redirectFmt, req->host, url);
		url = urlbuf;
	}
...
void send_r_not_found(request * req)
{
    ...
    if (req->method != M_HEAD) {
			#ifdef _IQv2_   //patrick modify. if 404 html file size smaller then 512bytes, IE use own 404 page.
			#ifdef _WIFI_REPEATER_
				req_write(req, "<html><head></head><script type=\"text/javascript\">window.top.location.assign(\"http://edimaxext.setup/\")</script><body><H1>404 Not Found</H1>\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n</body></html>");
			#else
				 req_write(req, "<html><head></head><script type=\"text/javascript\">window.top.location.assign(\"http://edimax.setup/\")</script><body><H1>404 Not Found</H1>\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n\n           The requested URL was not found on this server.           \n</body></html>");
			#endif
			#else
