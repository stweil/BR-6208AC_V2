...
/* $Id: read.c,v 1.1 2010/02/09 13:53:03 bradhuang Exp $*/
...
int read_header(request * req)
{
	...
	memset(req->soap_Action, 0x00, 128);
#endif
...
// davidhsu -------------------
	...
	if (req->upload_data) {
		...
		if (req->upload_len > (MAX_UPLOAD_SIZE/2) && !req->daemon_killed) {
			extern void kill_daemon();
			extern int check_auth_flag;
			if(check_auth_flag ==2){
				kill_daemon(1);
				req->daemon_killed = 1;
			}
		}	
...
// davidhsu    
     ...
     bytes_written = write(req->post_data_fd,
                          req->header_line, bytes_to_write);
#else
	if (strstr(req->request_uri, "/goform/formAdvFirewall")){
		if ((req->post_data_len + bytes_to_write)  > 13312) {
		...
		printf("error, post_data_len=%d > BUFFER_SIZE!\n", (req->post_data_len + bytes_to_write));
		return -1;		
		}	
	}
