...
/* $Id: boa.c,v 1.1 2010/02/09 13:53:03 bradhuang Exp $*/

#include "boa.h"
#include <signal.h>	//EDX jerry
...
char	*web_pidfile="/var/run/webs.pid"; // Brad add 
...
int main(int argc, char *argv[])
{
    ...
    FILE *fp;
    char line[20];
    extern void create_chklist_file(int); // davidhsu
...
#ifdef X86_SIMULATE
	getcwd(directory, 80);
	sprintf(dir, "%s/setting.bin", directory);
#endif

#ifdef SUPPORT_ASP
	extern void websAspInit(int argc,char **argv); // davidhsu
...
//	create_version_file(); // davidhsu   
//	create_chklist_file(0); // davidhsu
    ...
    default:
		...
		sprintf(line, "%d\n", getpid());
		if ((fp = fopen(web_pidfile, "w")) != NULL) {
			fwrite(line, strlen(line), 1, fp);
			fclose(fp);
		}
		boa_start = 1;
	init_signals(); //Brad move here
	signal(SIGUSR1, resetWebs);		//EDX jerry
    drop_privs();
    /* main loop */
    timestamp();
