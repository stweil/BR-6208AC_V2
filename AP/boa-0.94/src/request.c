...
/* $Id: request.c,v 1.1 2010/02/09 13:53:03 bradhuang Exp $*/
...
*/
//Brad modify for window.open issue--2006/10/23
//extern void checkfirmware_url(char *url);
//--------------------------------------------------
...
#ifdef SUPPORT_URL_LOGIN
extern int is_valid_user(request *wp);

static int check_login(request *req)
{
	int block=1, found_uri=0;
	char *p;
	//char buffer[200]; //Brad modify
	...
	int bypass = 0;
	if(strstr(req->request_uri, "active_sessions.asp") || strstr(req->request_uri, "cgi_ssi_igmp_group_memberships.asp") ||
		strstr(req->request_uri, "dhcp_clients.asp") || strstr(req->request_uri, "get_auto_wepkey.asp") ||
		strstr(req->request_uri, "get_ddns_status.asp") || strstr(req->request_uri, "get_wifisc_pin.asp") ||
		strstr(req->request_uri, "interface_stats.asp") || strstr(req->request_uri, "interface_stats_nowlan.asp") ||
		strstr(req->request_uri, "ssi_dns_resolve.asp") || strstr(req->request_uri, "ssi_ping_test.asp") ||
		strstr(req->request_uri, "wan_button_action.asp") || strstr(req->request_uri, "wan_connection_status.asp") ||
		strstr(req->request_uri, "wifi_assoc.asp") || strstr(req->request_uri, "wifi_log.asp") ||
		strstr(req->request_uri, "wifisc_add_sta.asp") || strstr(req->request_uri, "wifisc_add_sta_init.asp") ||
		strstr(req->request_uri, "wifisc_add_sta_progress.asp") || strstr(req->request_uri, "wifisc_ap_get_wpa_settings.asp") ||
		strstr(req->request_uri, "wifisc_cancel_add_sta.asp") || strstr(req->request_uri, "wifisc_pin.asp") || strstr(req->request_uri, "logout.asp") ||
		strstr(req->request_uri, "/HNAP") || strstr(req->request_uri, "dyn_clients_only.asp") || strstr(req->request_uri, "autoupgrade.asp")
	)
	{
		bypass = 1;
	}
	if (bypass == 0 && !is_valid_user(req)) {
		if (strstr(req->request_uri, ".asp") || strstr(req->request_uri, ".html") || 
			strstr(req->request_uri, "goform") || strstr(req->request_uri, WEB_PAGE_CFG_FILE) ||
			strstr(req->request_uri, WEB_PAGE_LOG_FILE)) 
		{
			if (strstr(req->request_uri, "formLogin") ||
				strstr(req->request_uri, WEB_PAGE_LOGIN) ||
				strstr(req->request_uri, WEB_PAGE_HOME) ||
				strstr(req->request_uri, WEB_PAGE_APPLY_OK) || 
				strstr(req->request_uri, WEB_PAGE_REBOOT) ||							
				strstr(req->request_uri, "formLanguageChange")	
			)
			{
				block = 0;
			}
				...
				if (strstr(req->request_uri, ".asp") || strstr(req->request_uri, ".html")) {
					if (strstr(req->request_uri, "login"))
						block=0;
					else {
						if (strstr(req->request_uri, "wps_back_result.asp")){
							 if(check_auth_flag==0){ //not login yet
							 	block=0;
							} 
						}else{
								strcpy(last_url, req->request_uri);
						   	    	found_uri = 1;
						}
					}
				}
			...
			if (block && !found_uri) {			
				if (strstr(req->request_uri, WEB_PAGE_CFG_FILE))
					strcpy(last_url, WEB_PAGE_SAVE_SETTING);
				else if (strstr(req->request_uri, FORM_FW_CHECK))
					strcpy(last_url, WEB_PAGE_FW_CHECK);
				else if (strstr(req->request_uri, FORM_FW_UPLOAD))
					strcpy(last_url, WEB_PAGE_FW_UPLOAD);
				else if (strstr(req->request_uri, FORM_CFG_UPLOAD))
					strcpy(last_url, WEB_PAGE_CFG_UPLOAD);
				else if (strstr(req->request_uri, FORM_CFG_SAVE))
					strcpy(last_url, WEB_PAGE_CFG_SAVE);
            #ifdef _Edimax_
				else if (strstr(req->request_uri, FORM_SAVE_TEXT))
					strcpy(last_url, WEB_PAGE_SAVE_TEXT);
            #endif
				else {		
					//fprintf(stderr, "<<<< DEBUG 1 the block case last_url=%s >>>\n",last_url);			
					p=strstr(req->header_line, "webpage=");
					//last_url[0]=0;
                        	memset(last_url, '\0', sizeof(last_url));
					...
					if (!last_url[0])					
						sprintf(last_url, "/%s", directory_index);
					//fprintf(stderr, "<<<< DEBUG 2 the block case last_url=%s >>>\n",last_url);	
				}
			}
			if (block) {
				send_redirect_perm(req, WEB_PAGE_LOGIN);
...
void process_requests(int server_sock)
{
    ...
    while (current) {
        ...
        case 0:                /* request complete */
           ...
           //brad add
#if 0                      
            	if(isFWUPGRADE == 1 && (!strcmp(current->request_uri,"/goform/formFirmwareUpgrade") || !strcmp(current->request_uri,"/goform/formLanguageUpgrade"))) {
            	firmware_len= current->upload_len;   //assign upload length
		firmware_data = current->upload_data; //assign upload data pointer
            	trailer = current;
            	current = current->next;
...
#if 0
#ifdef SUPPORT_ASP
	if (strstr(req->request_uri, "goform"))
...
#if defined(SUPPORT_HNAP)
//if ((req->method == M_GET || req->method == M_POST) && (strncasecmp(req->request_uri, "/HNAP", 5) == 0)) {
if ((req->method == M_POST) && (strncasecmp(req->request_uri, "/HNAP", 5) == 0)) {
	char admin_name[50]={0};
	char admin_password[50]={0};
	hnap_auth_flag = 0;
	
		apmib_get(MIB_ADMIN_NAME, admin_name);		
		apmib_get(MIB_ADMIN_PASSWORD, admin_password);	

		if (hanp_admin_name[0]) {
			if (!strcmp(hanp_admin_name, admin_name)) {
				if (admin_password[0]==0) {
						hnap_auth_flag = 2;
				}
				else  {
					if (!strcmp(hnap_admin_password, admin_password))
						hnap_auth_flag = 2;						
					}
				}				
			}			
...
#else
	if (strstr(req->logline, "/goform/formAdvFirewall")){
			req->post_data =  calloc(1, 13312);
    	}else{
...
#ifdef SUPPORT_ASP
	#ifdef _Edimax_
		if (strstr(req->logline, FORM_FW_UPLOAD) || strstr(req->logline, FORM_SAVE_TEXT) || strstr(req->logline, FORM_CFG_SAVE) || strstr(req->logline, FORM_CFG_UPLOAD) || strstr(req->logline,FORM_LANGUAGE_UPLOAD)) {
	#else
		if (strstr(req->logline, FORM_FW_UPLOAD) || strstr(req->logline, FORM_CFG_SAVE) || strstr(req->logline, FORM_CFG_UPLOAD) || strstr(req->logline,FORM_LANGUAGE_UPLOAD)) {	
	#endif    
...
// davidhsu, direct reboot page to ramdisk image ----
//	if (!strcmp(req->request_uri, WEB_PAGE_REBOOT)) {
//		sprintf(req->pathname, "/var/tmp%s", WEB_PAGE_REBOOT);
//	}
	
#ifdef SUPPORT_ASP
	if (strstr(req->request_uri, ".asp") || !strcmp(req->request_uri, "/"))
		req->cgi_type = 1;
#if defined(SUPPORT_HNAP)
		else if ((strncasecmp(req->request_uri, "/HNAP", 5) == 0)) {
				req->cgi_type = 1;
		}
#endif
#endif
...
int process_option_line(request * req)
{
    ...
    /* Start by aggressively hacking the in-place copy of the header line */
   	...
   	if(line[0]){
		if(strstr(line, "SOAPAction:")){
			char *Action_tok;
			int Action_len=0;
			Action_tok=strstr(line, "HNAP1/");
			Action_tok = Action_tok+6;
			Action_len = strlen(Action_tok);
			memcpy(req->soap_Action, Action_tok, Action_len-1);
			req->soap_Action[Action_len-1] = '\0';
		}
	}
#endif
